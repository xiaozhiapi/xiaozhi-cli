/**
 * XIAOZHI Plugin Hooks for OpenCode
 *
...
/**
 * Read XIAOZHI version from package.json
 * Falls back to a default if package.json cannot be read
 */
function getXiaozhiVersion(): string {
  try {
...
type XiaozhiHooksPluginFn = (input: PluginInput) => Promise<Record<string, unknown>>

export const XiaozhiHooksPlugin: XiaozhiHooksPluginFn = async ({
  client,
  $,
  directory,
  worktree,
  ...
  const log = (level: "debug" | "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "xiaozhi", level, message } })
  ...
  const currentProfile = normalizeProfile(process.env.XIAOZHI_HOOK_PROFILE)
  const disabledHooks = new Set(
    (process.env.XIAOZHI_DISABLED_HOOKS || "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
  )
  ...
  return {
    ...
    "file.edited": async (event: { path: string }) => {
      ...
      if (hookEnabled("post:edit:format", ["strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          log("info", `[XIAOZHI] Formatted: ${event.path}`)
        } catch (error: unknown) {
          // Prettier not installed or failed - log but continue
          const errorMessage = error instanceof Error ? error.message : String(error)
          log("debug", `[XIAOZHI] Prettier formatting failed for ${event.path}: ${errorMessage}`)
        }
      }
      ...
      if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          if (result.trim()) {
            ...
            log(
              "warn",
              `[XIAOZHI] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`
            )
          }
        } catch {
    ...
    "tool.execute.after": async (
      ...
      if (
        ...
        try {
          ...
          log("info", "[XIAOZHI] TypeScript check passed")
        } catch (error: unknown) {
          const err = error as { stdout?: string }
          log("warn", "[XIAOZHI] TypeScript errors detected:")
          if (err.stdout) {
      ...
      if (
        ...
        log("info", "[XIAOZHI] PR created - check GitHub Actions status")
      }
    },
    ...
    "tool.execute.before": async (
      ...
      if (
        ...
        log(
          "info",
          "[XIAOZHI] Remember to review changes before pushing: git diff origin/main...HEAD"
        )
      }
      ...
      if (
        ...
        if (
          ...
          log(
            "warn",
            `[XIAOZHI] Creating ${filePath} - consider if this documentation is necessary`
          )
        }
      }
      ...
      if (hookEnabled("pre:bash:tmux-reminder", "strict") && input.tool === "bash") {
        ...
        if (
          ...
          log(
            "info",
            "[XIAOZHI] Long-running command detected - consider using background execution"
          )
        }
      }
    },
    ...
    "session.created": async () => {
      ...
      log("info", `[XIAOZHI] Session started - profile=${currentProfile}`)

      // Check for project-specific context files
      if (hasProjectFile("CLAUDE.md")) {
        log("info", "[XIAOZHI] Found CLAUDE.md - loading project context")
      }
    },
    ...
    "session.idle": async () => {
      ...
      log("info", "[XIAOZHI] Session idle - running console.log audit")
      ...
      if (totalConsoleLogCount > 0) {
        log(
          "warn",
          `[XIAOZHI] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`
        )
        filesWithConsoleLogs.forEach((f) =>
          log("warn", `  - ${f}`)
        )
        log("warn", "[XIAOZHI] Remove console.log statements before committing")
      } else {
        log("info", "[XIAOZHI] Audit passed: No console.log statements found")
      }
      ...
      try {
        if (process.platform === "darwin") {
          // macOS
          await $`osascript -e 'display notification "Task completed!" with title "OpenCode XIAOZHI"' 2>/dev/null`
        } else if (process.platform === "win32") {
          // Windows - PowerShell notification
          await $`powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Task completed!', 'OpenCode XIAOZHI', 'OK', 'Information')" 2>/dev/null`
        } else if (process.platform === "linux") {
          // Linux - notify-send (requires libnotify)
          await $`notify-send "OpenCode XIAOZHI" "Task completed!" 2>/dev/null`
        }
      } catch (error: unknown) {
        // Notification not supported or failed - log but continue
        const errorMessage = error instanceof Error ? error.message : String(error)
        log("debug", `[XIAOZHI] Desktop notification failed: ${errorMessage}`)
      }
    ...
    "session.deleted": async () => {
      ...
      log("info", "[XIAOZHI] Session ended - cleaning up")
      editedFiles.clear()
      clearChanges()
    ...
    "todo.updated": async (event: { todos: Array<{ text: string; done: boolean }> }) => {
      ...
      if (total > 0) {
        log("info", `[XIAOZHI] Progress: ${completed}/${total} tasks completed`)
      }
    },
     ...
     * Action: Sets PROJECT_ROOT, PACKAGE_MANAGER, DETECTED_LANGUAGES, XIAOZHI_VERSION
     */
    "shell.env": async () => {
      const env: Record<string, string> = {
        XIAOZHI_VERSION: getXiaozhiVersion(),
        XIAOZHI_PLUGIN: "true",
        XIAOZHI_HOOK_PROFILE: currentProfile,
        XIAOZHI_DISABLED_HOOKS: process.env.XIAOZHI_DISABLED_HOOKS || "",
     ...
     * Action: Push XIAOZHI context block and custom compaction prompt
     */
    "experimental.session.compacting": async () => {
      const contextBlock = [
        "# XIAOZHI Context (preserve across compaction)",
        "",
        "## Active Plugin: XIAOZHI v2.0.0",
    ...
    "permission.ask": async (event: PermissionEvent) => {
      log("info", `[XIAOZHI] Permission requested for: ${event.tool}`)
      ...
      try {
        ...
        if (["read", "glob", "grep", "search", "list"].includes(event.tool)) {
          log("debug", `[XIAOZHI] Auto-approved read-only tool: ${event.tool}`)
        ...
        if (event.tool === "bash" && /^(npx )?(@biomejs\/biome|prettier|black|gofmt|rustfmt|swift-format)/.test(cmd)) {
          log("debug", `[XIAOZHI] Auto-approved formatter: ${cmd}`)
        ...
        if (event.tool === "bash" && /^(npm test|npx vitest|npx jest|pytest|go test|cargo test)/.test(cmd)) {
          log("debug", `[XIAOZHI] Auto-approved test execution: ${cmd}`)
          return { approved: true, reason: "Test execution" }
        }

        // Everything else: let user decide
        log("debug", `[XIAOZHI] Permission requires user approval: ${event.tool}`)
        return { approved: undefined }
      } catch (error: unknown) {
        // Error in permission handling - log and deny for safety
        const errorMessage = error instanceof Error ? error.message : String(error)
        log("error", `[XIAOZHI] Permission handling error for ${event.tool}: ${errorMessage}`)
...
export default XiaozhiHooksPlugin
