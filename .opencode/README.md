# OpenCode XIAOZHI Plugin

> WARNING: This README is specific to OpenCode usage.
> If you installed XIAOZHI via npm (e.g. `npm install opencode-xiaozhi`), refer to the root README instead.

XIAOZHI plugin for OpenCode - agents, commands, hooks, and skills.

## Installation

## Installation Overview

There are two ways to use XIAOZHI:

1. **npm package (recommended for most users)**
   Install via npm/bun/yarn and use the `xiaozhi-install` CLI to set up rules and agents.
...
This loads the XIAOZHI OpenCode plugin module from npm:
- hook/event integrations
- bundled custom tools exported by the plugin

It does **not** auto-register the full XIAOZHI command/agent/instruction catalog in your project config. For the full OpenCode setup, either:
...
After installation, the `xiaozhi-install` CLI is also available:

```bash
npx xiaozhi-install typescript
```
...
If you also want to apply the XIAOZHI home install
...
- `XIAOZHI_HOOK_PROFILE`: `minimal`, `standard` (default), `strict`
- `XIAOZHI_DISABLED_HOOKS`: comma-separated hook IDs to disable

## Skills

The default OpenCode config loads 11 curated XIAOZHI skills via the `instructions` array:
