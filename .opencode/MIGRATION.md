...
This guide helps you migrate from Claude Code to OpenCode while using the XIAOZHI configuration.
...
### XIAOZHI Plugin Hooks Included

The XIAOZHI OpenCode configuration includes translated hooks:
...
### 2. Use the XIAOZHI OpenCode Configuration
...
│   ├── xiaozhi-hooks.ts           # All XIAOZHI hooks as plugins
...
### Option 1: Use XIAOZHI Configuration Directly
...
This only loads the published XIAOZHI OpenCode plugin module (hooks/events and exported plugin tools).
It does **not** automatically inject XIAOZHI's full `agent`, `command`, or `instructions` config into your project.

If you want the full XIAOZHI OpenCode workflow surface, use the repository's bundled `.opencode/opencode.json` as your base config or copy these pieces into your project:
- `.opencode/commands/`
- `.opencode/prompts/`
...
4. If you installed only `plugin: ["xiaozhi-cli"]`, note that npm plugin install does not auto-add XIAOZHI commands or agents to your project config
...
- **XIAOZHI Configuration**: Report to [github.com/xiaozhiapi/xiaozhi-cli](https://github.com/xiaozhiapi/xiaozhi-cli)
