...
do_uninstall() {
    ...
    echo "XIAOZHI CodeBuddy Uninstaller"
    echo "=========================="
    echo ""
    ...
    if [ ! -f "$MANIFEST" ]; then
        echo "Warning: No manifest file found (.xiaozhi-manifest)"
        echo ""
        echo "This could mean:"
        echo "  1. XIAOZHI was installed with an older version without manifest support"
    ...
    echo "Found manifest file - will only remove files installed by XIAOZHI"
    echo ""
    read -p "Are you sure you want to uninstall XIAOZHI from $CODEBUDDY_DIR? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
