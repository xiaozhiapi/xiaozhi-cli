...
# Locate the XIAOZHI repo root by walking up from SCRIPT_DIR to find the marker
...
if [ -z "$REPO_ROOT" ]; then
    echo "Error: Cannot locate the XIAOZHI repository root."
    echo "This script must be run from within the XIAOZHI repository's .codebuddy/ directory."
    exit 1
fi
...
do_install() {
    ...
    echo "XIAOZHI CodeBuddy Installer"
    echo "======================="
    echo ""
    ...
    MANIFEST="$codebuddy_full_path/.xiaozhi-manifest"
    touch "$MANIFEST"
    ...
    ensure_manifest_entry "$MANIFEST" ".xiaozhi-manifest"
    ...
    echo "  3. Enjoy the XIAOZHI workflows!"
    echo ""
