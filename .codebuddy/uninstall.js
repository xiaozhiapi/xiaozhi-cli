#!/usr/bin/env node
/**
 * XIAOZHI CodeBuddy Uninstaller (Cross-platform Node.js version)
...
async function doUninstall() {
  ...
  console.log('XIAOZHI CodeBuddy Uninstaller');
  console.log('==========================');
  console.log('');
  ...
  const manifest = path.join(codebuddyFullPath, '.xiaozhi-manifest');

  // Handle missing manifest
  if (!fs.existsSync(manifest)) {
    console.log('Warning: No manifest file found (.xiaozhi-manifest)');
    console.log('');
    console.log('This could mean:');
    console.log('  1. XIAOZHI was installed with an older version without manifest support');
  ...
  console.log('Found manifest file - will only remove files installed by XIAOZHI');
  console.log('');

  const confirmed = await promptConfirm(`Are you sure you want to uninstall XIAOZHI from ${codebuddyDirName}? (y/N) `);
  if (!confirmed) {
    console.log('Uninstall cancelled.');
    process.exit(0);
  }
