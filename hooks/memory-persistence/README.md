...
These lifecycle hook definitions document XIAOZHI's memory persistence contract for Claude Code plugin and manual installs.
...
- Bound context loaded at session start with `XIAOZHI_SESSION_START_MAX_CHARS`.
- Allow opt-out with `XIAOZHI_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `XIAOZHI_HOOK_PROFILE` and `XIAOZHI_DISABLED_HOOKS`.
