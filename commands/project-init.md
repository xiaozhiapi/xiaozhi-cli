---
description: Detect a project's stack and produce a dry-run XIAOZHI onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable XIAOZHI onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
/project-init --config xiaozhi-install.json
```
...
3. Use XIAOZHI's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- XIAOZHI config: `xiaozhi-install.json`
- optional stack map: `config/project-stack-mappings.json` in the XIAOZHI repo

When the XIAOZHI checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed XIAOZHI manifests and explicit user choices.
...
3. Resolve the smallest useful XIAOZHI plan:
   - project has an `xiaozhi-install.json`: `node scripts/install-plan.js --config xiaozhi-install.json --json`
...
- `/xiaozhi-guide` for interactive feature discovery before installing
