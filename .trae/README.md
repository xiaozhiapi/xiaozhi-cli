...
Bring Everything Claude Code (XIAOZHI) workflows to Trae IDE. This repository provides custom commands, agents, skills, and rules that can be installed into any Trae project with a single command.
...
This creates `/path/to/your/project/.trae-cn/` with all XIAOZHI components.
...
This creates `~/.trae-cn/` with all XIAOZHI components. All Trae projects will use these global installations.

**Note**: Global installation is useful when you want to maintain a single copy of XIAOZHI across all your projects.
...
The uninstaller uses a manifest file (`.xiaozhi-manifest`) to track installed files, ensuring safe removal:
...
- **Safe removal**: Only removes files tracked in the manifest (installed by XIAOZHI)
...
- **Manifest-based**: Requires `.xiaozhi-manifest` file (created during install)
