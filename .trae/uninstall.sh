...
do_uninstall() {
    ...
    # Manifest file path
    MANIFEST="$trae_full_path/.xiaozhi-manifest"
    ...
    echo ""
    read -p "Are you sure you want to uninstall XIAOZHI from $trae_dir? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
