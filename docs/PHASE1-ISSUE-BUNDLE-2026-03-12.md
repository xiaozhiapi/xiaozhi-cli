...
- `#423` Implement manifest-driven selective install profiles for XIAOZHI
- `#421` Add XIAOZHI install-state plus uninstall / doctor / repair lifecycle
- `#424` Define canonical session adapter contract for XIAOZHI 2.0 control plane
...
Implement manifest-driven selective install profiles for XIAOZHI
...
XIAOZHI still installs primarily by target and language. The repo now has first-pass
...
Implement manifest-driven install execution for current XIAOZHI targets:
...
- `xiaozhi-install --profile <name>`
- `xiaozhi-install --modules <id,id,...>`
...
Add XIAOZHI install-state plus uninstall / doctor / repair lifecycle
...
XIAOZHI has no canonical installed-state record. That makes uninstall, repair, and
...
- what paths XIAOZHI owns
- how to remove or repair only XIAOZHI-managed files
...
- Claude: `~/.claude/xiaozhi/install-state.json`
- Cursor: `./.cursor/xiaozhi-install-state.json`
- Antigravity: `./.agent/xiaozhi-install-state.json`
...
- `uninstall` removes only XIAOZHI-managed files and leaves unrelated local files
  alone
...
Define canonical session adapter contract for XIAOZHI 2.0 control plane
...
XIAOZHI now has real orchestration/session substrate, but it is still
...
Without that adapter contract, any future XIAOZHI 2.0 operator shell will be forced
...
- Full XIAOZHI 2.0 UI in the same issue
...
XIAOZHI now has a large and growing skill surface, but generated/imported/learned
...
As XIAOZHI grows, the repo needs explicit rules for where generated skill artifacts
