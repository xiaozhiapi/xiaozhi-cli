...
* 为 Hermes 操作员叙事新增公开的 XIAOZHI 2.0 release candidate 表面。
* 将 XIAOZHI 明确记录为跨 Claude Code、Codex、Cursor、OpenCode 和 Gemini 的可复用 cross-harness 基础层。
...
* 这是 release candidate，不是完整 XIAOZHI 2.0 control-plane 路线图的 GA 声明。
...
* 这是仓库表面校正和生态同步版本，不表示完整 XIAOZHI 2.0 路线图已经完成。
...
* `blueprint` — 多会话构建规划
* `everything-claude-code` — 自引用 XIAOZHI 技能 (#335)
...
* 增加了 `XIAOZHI_HOOK_PROFILE` 和 `XIAOZHI_DISABLED_HOOKS` 控制。
