...
[![Stars](https://img.shields.io/github/stars/xiaozhiapi/everything-claude-code?style=flat)](https://github.com/xiaozhiapi/everything-claude-code/stargazers)
[![Forks](https://img.shields.io/github/forks/xiaozhiapi/everything-claude-code?style=flat)](https://github.com/xiaozhiapi/everything-claude-code/network/members)
[![Contributors](https://img.shields.io/github/contributors/xiaozhiapi/everything-claude-code?style=flat)](https://github.com/xiaozhiapi/everything-claude-code/graphs/contributors)
[![npm xiaozhi-cli](https://img.shields.io/npm/dw/xiaozhi-cli?label=xiaozhi-cli%20weekly%20downloads\&logo=npm)](https://www.npmjs.com/package/xiaozhi-cli)
[![npm xiaozhi-agentshield](https://img.shields.io/npm/dw/xiaozhi-agentshield?label=xiaozhi-agentshield%20weekly%20downloads\&logo=npm)](https://www.npmjs.com/package/xiaozhi-agentshield)
[![GitHub App Install](https://img.shields.io/endpoint?url=https%3A%2F%2Fgithub.com/xiaozhiapi/xiaozhi-cli%2Fbadge%2Finstalls&logo=github)](https://github.com/xiaozhiapi/xiaozhi-cli)
...
2.0 主线稳定版：261 个技能、control-pane 基底（会话适配器 + MCP 清单）、worktree 生命周期服务，以及 [XIAOZHI Discord 社区](https://discord.gg/36yGMHGFbR)。

### v2.0.0-rc.1 — 表面同步、运营工作流与 XIAOZHI 2.0 Alpha（2026年4月）
...
* **平台优先发布** — XIAOZHI 现在被明确构建为一个智能体平台性能系统，而不仅仅是一个配置包。
* **钩子可靠性大修** — SessionStart 根回退、Stop 阶段会话摘要，以及用基于脚本的钩子替换脆弱的单行内联钩子。
* **钩子运行时控制** — `XIAOZHI_HOOK_PROFILE=minimal|standard|strict` 和 `XIAOZHI_DISABLED_HOOKS=...` 用于运行时门控，无需编辑钩子文件。
...
* **GitHub 市场** — XIAOZHI Tools GitHub 应用已在 [github.com/xiaozhiapi/xiaozhi-cli](https://github.com/xiaozhiapi/xiaozhi-cli) 上线，提供免费/专业/企业版
* **合并了 30+ 个社区 PR** — 来自 6 种语言的 30 位贡献者的贡献
* **978项内部测试** — 在代理、技能、命令、钩子和规则方面扩展了验证套件

### v1.4.1 — 错误修复 (2026年2月)

* **修复了直觉导入内容丢失问题** — `parse_instinct_file()` 在 `/instinct-import` 期间会静默丢弃 frontmatter 之后的所有内容（Action, Evidence, Examples 部分）。已由社区贡献者 @ericcai0814 修复 ([#148](https://github.com/xiaozhiapi/everything-claude-code/issues/148), [#161](https://github.com/xiaozhiapi/everything-claude-code/pull/161))

### v1.4.0 — 多语言规则、安装向导 & PM2 (2026年2月)

* **交互式安装向导** — 新的 `configure-xiaozhi` 技能提供了带有合并/覆盖检测的引导式设置
...
完整的更新日志请参见 [Releases](https://github.com/xiaozhiapi/everything-claude-code/releases)。
...
```bash
# Add marketplace
/plugin marketplace add https://github.com/xiaozhiapi/xiaozhi-cli

# Install plugin
/plugin install xiaozhi@xiaozhi
```
...
>
> 如果你已经通过 `/plugin install` 安装了 XIAOZHI，**不要再运行 `./install.sh --profile full`、`.\install.ps1 --profile full` 或 `npx xiaozhi-install --profile full`**。插件已经会自动加载 XIAOZHI 的技能、命令和 hooks；此时再执行完整安装，会把同一批内容再次复制到用户目录，导致技能重复以及运行时行为重复。
>
> 对于插件安装路径，请只手动复制你需要的 `rules/` 目录。只有在你完全不走插件安装、而是选择“纯手动安装 XIAOZHI”时，才应该使用完整安装器。

```bash
# Clone the repo first
git clone https://github.com/xiaozhiapi/everything-claude-code.git
...
# Fully manual XIAOZHI install path (do this instead of /plugin install)
...
# npx xiaozhi-install --profile full
```
...
/xiaozhi:plan "Add user authentication"

# Manual install (Option 2) uses the shorter form:
# /plan "Add user authentication"

# Check available commands
/plugin list xiaozhi@xiaozhi
```
...
export XIAOZHI_HOOK_PROFILE=standard

# Comma-separated hook IDs to disable
export XIAOZHI_DISABLED_HOOKS="pre:bash:tmux-reminder,post:edit:typecheck"
```
...
[安装 GitHub 应用](https://github.com/apps/skill-creator) | [github.com/xiaozhiapi/xiaozhi-cli](https://github.com/xiaozhiapi/xiaozhi-cli)
...
npx xiaozhi-agentshield scan

# Auto-fix safe issues
npx xiaozhi-agentshield scan --fix

# Deep analysis with three Opus 4.6 agents
npx xiaozhi-agentshield scan --opus --stream

# Generate secure config from scratch
npx xiaozhi-agentshield init
```
...
在 Claude Code 中使用 `/security-scan` 来运行它，或者通过 [GitHub Action](https://github.com/xiaozhiapi/agentshield) 添加到 CI。

[GitHub](https://github.com/xiaozhiapi/agentshield) | [npm](https://www.npmjs.com/package/xiaozhi-agentshield)
...
**历史背景：** 这已导致此仓库中多次修复/还原循环（[#29](https://github.com/xiaozhiapi/everything-claude-code/issues/29), [#52](https://github.com/xiaozhiapi/everything-claude-code/issues/52), [#103](https://github.com/xiaozhiapi/everything-claude-code/issues/103)）。Claude Code 版本之间的行为发生了变化，导致了混淆。我们现在有一个回归测试来防止这种情况再次发生。
...
```json
{
  "extraKnownMarketplaces": {
    "xiaozhi": {
      "source": {
        "source": "github",
        "repo": "xiaozhiapi/everything-claude-code"
      }
    }
  },
  "enabledPlugins": {
    "xiaozhi@xiaozhi": true
  }
}
```
...
| 我想要... | 使用此表面 | 使用的智能体 |
|--------------|-----------------|------------|
| 规划新功能 | `/xiaozhi:plan "Add auth"` | planner |
| 设计系统架构 | `/xiaozhi:plan` + architect agent | architect |
...
```
/xiaozhi:plan "使用 OAuth 添加用户身份验证"
                                              → 规划器创建实现蓝图
...
这是最常见的问题。**不要在 `.claude-plugin/plugin.json` 中添加 `"hooks"` 字段。** Claude Code v2.1+ 会自动从已安装的插件加载 `hooks/hooks.json`。显式声明它会导致重复检测错误。参见 [#29](https://github.com/xiaozhiapi/everything-claude-code/issues/29), [#52](https://github.com/xiaozhiapi/everything-claude-code/issues/52), [#103](https://github.com/xiaozhiapi/everything-claude-code/issues/103)。

</details>

<details>
<summary><b>我能否在自定义API端点或模型网关上使用XIAOZHI与Claude Code？</b></summary>

是的。XIAOZHI 不会硬编码 Anthropic 托管的传输设置。它通过 Claude Code 正常的 CLI/插件接口在本地运行，因此可以与以下系统配合工作：
...
如果您的网关重新映射模型名称，请在 Claude Code 中配置，而不是在 XIAOZHI 中。一旦 `claude` CLI 已经正常工作，XIAOZHI 的钩子、技能、命令和规则就与模型提供商无关。
...
* **Codex**: 对 macOS 应用和 CLI 的一流支持，带有适配器漂移防护和 SessionStart 回退。参见 PR [#257](https://github.com/xiaozhiapi/everything-claude-code/pull/257)。
...
XIAOZHI 提供**完整的 Cursor IDE 支持**，包括为 Cursor 原生格式适配的钩子、规则、代理、技能、命令和 MCP 配置。
...
XIAOZHI 为 macOS 应用和 CLI 提供 **一流的 Codex 支持**，包括参考配置、Codex 特定的 AGENTS.md 补充文档以及共享技能。
...
| everything-claude-code | XIAOZHI 项目的开发约定和模式 |
...
Codex **尚未提供与 Claude 风格同等的钩子执行功能**。XIAOZHI 在该平台上的强制执行是通过 `AGENTS.md`、可选的 `model_instructions_file` 覆盖以及沙箱/批准设置以指令方式实现的。
...
XIAOZHI 提供 **完整的 OpenCode 支持**，包括插件和钩子。
...
该 npm 插件条目启用了 XIAOZHI 发布的 OpenCode 插件模块（钩子/事件和插件工具）。
它**不会**自动将 XIAOZHI 的完整命令/代理/指令目录添加到您的项目配置中。

要获得完整的 XIAOZHI OpenCode 设置，您可以：
...
XIAOZHI 是**第一个最大化利用每个主要 AI 编码工具的插件**。以下是每个平台的比较：
...
[**成为赞助商**](https://github.com/xiaozhiapi/xiaozhi-cli) | [赞助层级](SPONSORS.md) | [赞助计划](SPONSORING.md)

***

## Star 历史

[![Star History Chart](https://api.star-history.com/svg?repos=xiaozhiapi/everything-claude-code\&type=Date)](https://star-history.com/#xiaozhiapi/everything-claude-code\&Date)
