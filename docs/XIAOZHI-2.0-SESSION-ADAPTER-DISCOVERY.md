# XIAOZHI 2.0 Session Adapter Discovery

## Purpose

This document turns the March 11 XIAOZHI 2.0 control-plane direction into a
...
- `scripts/lib/session-adapters/canonical-session.js`
  defines the canonical `xiaozhi.session.v1` normalization layer
...
In practice, XIAOZHI can already answer:
...
general XIAOZHI 2.0 control plane.
...
## Gap Between XIAOZHI 1.x And XIAOZHI 2.0

XIAOZHI 1.x currently has two different "session" surfaces:
...
The missing XIAOZHI 2.0 layer is a harness-neutral session adapter boundary that
can normalize:
...
XIAOZHI 2.0 should introduce a canonical session adapter contract.
...
This provides a non-orchestrated baseline for XIAOZHI 2.0.
...
3. Should event streaming be part of v1, or should XIAOZHI 2.0 ship snapshot-only
   first?
...
5. Should the adapter registry live inside this repo long-term, or move into the
   eventual XIAOZHI 2.0 control-plane app once the interface stabilizes?
...
The shortest path to XIAOZHI 2.0 is:
