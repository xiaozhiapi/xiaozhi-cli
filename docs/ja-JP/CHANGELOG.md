...
- HermesオペレーターストーリーのためのパブリックXIAOZHI 2.0リリース候補サーフェスを追加。
- Claude Code、Codex、Cursor、OpenCode、Gemini全体で再利用可能なクロスハーネス基盤としてXIAOZHIをドキュメント化。
...
- `docs/architecture/cross-harness.md` とXIAOZHI/Hermesバウンダリのリグレッションカバレッジを追加。
- `xiaozhi2/` のバージョニングは現時点では独立を維持；リリースエンジニアリングが別途決定しない限り、アルファコントロールプレーンのスキャフォールドのまま。

### 注記

- これはリリース候補であり、完全なXIAOZHI 2.0コントロールプレーンロードマップのGA宣言ではありません。
...
- XIAOZHI 2.0アルファコントロールプレーンバイナリが `xiaozhi2/` からローカルビルド可能になり、最初の使用可能なCLI/TUIサーフェスを公開。
...
- `cargo build --manifest-path xiaozhi2/Cargo.toml` がリポジトリのベースラインで通過。
- `xiaozhi-tui` は現在 `dashboard`、`start`、`sessions`、`status`、`stop`、`resume`、`daemon` を公開。
...
- このリリースはリポジトリサーフェスの修正とエコシステム同期であり、完全なXIAOZHI 2.0ロードマップが完成したという主張ではありません。
...
- `everything-claude-code` — 自己参照型XIAOZHIスキル (#335)
...
- `XIAOZHI_HOOK_PROFILE` と `XIAOZHI_DISABLED_HOOKS` 制御を追加。
