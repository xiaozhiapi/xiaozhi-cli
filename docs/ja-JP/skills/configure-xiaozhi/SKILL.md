...
- ユーザーが "configure xiaozhi"、"install xiaozhi"、"setup everything claude code" などと言った場合
- ユーザーがこのプロジェクトからスキルまたはルールを選択的にインストールしたい場合
- ユーザーが既存の XIAOZHI インストールを検証または修正したい場合
- ユーザーがインストールされたスキルまたはルールをプロジェクト用に最適化したい場合

## 前提条件

このスキルは起動前に Claude Code からアクセス可能である必要があります。ブートストラップには2つの方法があります：
1. **プラグイン経由**: `/plugin install xiaozhi@xiaozhi` — プラグインがこのスキルを自動的にロードします
2. **手動**: このスキルのみを `~/.claude/skills/configure-xiaozhi/SKILL.md` にコピーし、"configure xiaozhi" と言って起動します

---

## ステップ 0: XIAOZHI リポジトリのクローン

インストールの前に、最新の XIAOZHI ソースを `/tmp` にクローンします：
...
以降のすべてのコピー操作のソースとして `XIAOZHI_ROOT=/tmp/everything-claude-code` を設定します。

クローンが失敗した場合（ネットワークの問題など）、`AskUserQuestion` を使用してユーザーに既存の XIAOZHI クローンへのローカルパスを提供するよう依頼します。
...
```
Question: "XIAOZHI コンポーネントをどこにインストールしますか？"
Options:
...
cp -r $XIAOZHI_ROOT/rules/typescript $TARGET/rules/typescript   # 選択された場合
cp -r $XIAOZHI_ROOT/rules/python $TARGET/rules/python            # 選択された場合
cp -r $XIAOZHI_ROOT/rules/golang $TARGET/rules/golang            # 選択された場合
```
...
**重要**: インストール先（`$TARGET/`）のファイルのみを変更し、ソース XIAOZHI リポジトリ（`$XIAOZHI_ROOT/`）のファイルは決して変更しないでください。
