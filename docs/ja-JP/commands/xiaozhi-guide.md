---
description: XIAOZHIの現在のエージェント、スキル、コマンド、フック、インストールプロファイル、ドキュメントをライブリポジトリサーフェスからナビゲートします。
---

# /xiaozhi-guide

このコマンドをEverything Claude Codeの会話型マップとして使用します。完全なREADMEや古いカタログ数をダンプすることなく、タスクに適したXIAOZHIサーフェスをユーザーが発見するのを支援します。
...
/xiaozhi-guide find: <クエリ>
/xiaozhi-guide <機能名またはファイル名>
```
...
- `scripts/ci/catalog.js --json` — XIAOZHI内で実行時のライブカタログカウント
...
- `/project-init` — スタック対応のXIAOZHIオンボーディング
