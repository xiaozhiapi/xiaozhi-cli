# Hermes / OpenClaw -> XIAOZHI Migration

This document is the public migration guide for moving a Hermes or OpenClaw-style operator setup into the current XIAOZHI model.
...
XIAOZHI is the durable public system:
...
- install surfaces
- session adapters
- XIAOZHI 2.0 control-plane work

Hermes and OpenClaw are useful inputs because they contain repeated operator workflows that can be distilled into XIAOZHI-native surfaces.

That means the shortest safe path is:

1. extract the reusable behavior
2. translate it into XIAOZHI-native skills, hooks, docs, or adapter work
...
- XIAOZHI hook / command automation for local repeatability
- XIAOZHI 2.0 scheduler work under issue `#1050`
...
- XIAOZHI 2.0 as the future long-lived scheduling/control plane
...
- XIAOZHI session adapter and control-plane work
- orchestration/session inspection commands
- XIAOZHI 2.0 control-plane backlog under:
  - `#1045`
  - `#1046`
  - `#1047`
  - `#1048`
...
- XIAOZHI-native top-level skills when the workflow is reusable
...
- XIAOZHI-native operator skills when the workflow logic is the real asset
- adapter/control-plane work when the missing piece is session/runtime coordination

Do not import opaque third-party runtimes into XIAOZHI just because a private workflow depended on them.

If a workflow is valuable:

1. understand the behavior
2. rebuild the minimum XIAOZHI-native version
...
- XIAOZHI 2.0 adapter/control-plane discovery docs
...
- continuing the XIAOZHI 2.0 operator/control-plane buildout

XIAOZHI 2.0 now ships a bounded migration audit entrypoint:

- `xiaozhi migrate audit --source ~/.hermes`
- `xiaozhi migrate plan --source ~/.hermes --output migration-plan.md`
- `xiaozhi migrate scaffold --source ~/.hermes --output-dir migration-artifacts`
- `xiaozhi migrate import-skills --source ~/.hermes --output-dir migration-artifacts/skills`
- `xiaozhi migrate import-tools --source ~/.hermes --output-dir migration-artifacts/tools`
- `xiaozhi migrate import-plugins --source ~/.hermes --output-dir migration-artifacts/plugins`
- `xiaozhi migrate import-schedules --source ~/.hermes --dry-run`
- `xiaozhi migrate import-remote --source ~/.hermes --dry-run`
...
1. Keep the public XIAOZHI repo as the canonical reusable layer.
2. Port reusable Hermes/OpenClaw workflows into XIAOZHI-native skills one lane at a time.
3. Keep private auth and personal context outside the repo.
4. Use GitHub / Linear / KB systems as durable truth.
5. Treat XIAOZHI 2.0 as the path to a native operator shell, not as a finished product.
