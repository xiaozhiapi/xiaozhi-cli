...
This document defines the canonical XIAOZHI session snapshot contract for
`xiaozhi.session.v1`.
...
XIAOZHI has multiple session sources:
...
| Field | Type | Notes |
| --- | --- | --- |
| `schemaVersion` | string | MUST be exactly `xiaozhi.session.v1` for this contract |
...
Every XIAOZHI session adapter MUST:

1. Emit `schemaVersion: "xiaozhi.session.v1"` exactly.
...
- rely only on documented fields for `xiaozhi.session.v1`
