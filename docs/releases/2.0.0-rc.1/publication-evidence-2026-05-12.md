# XIAOZHI v2.0.0-rc.1 Publication Evidence — 2026-05-12
...
| --- | --- | --- |
| GitHub prerelease | `gh release view v2.0.0-rc.1 --repo xiaozhiapi/everything-claude-code --json tagName,url,isPrerelease` | `release not found` |
| npm dist-tags | `npm view xiaozhi-cli dist-tags --json` | `{ "latest": "1.10.0" }` |
| npm package metadata | `node -p "require('./package.json').name + '@' + require('./package.json').version"` | `xiaozhi-cli@2.0.0-rc.1` |
...
| `npm pack --dry-run --json` | `xiaozhi-cli-2.0.0-rc.1.tgz`; `entryCount: 965`; `size: 1565968`; `unpackedSize: 4934637`; `hasBytecode: false` |
...
| Command | Result |
| --- | --- |
| `npm pack --pack-destination /tmp/xiaozhi-publication-smoke-dd9ud5 --json` | Created `xiaozhi-cli-2.0.0-rc.1.tgz` for local install smoke |
| `npm install --prefix /tmp/xiaozhi-publication-smoke-dd9ud5 /tmp/xiaozhi-publication-smoke-dd9ud5/xiaozhi-cli-2.0.0-rc.1.tgz` | Added 8 packages |
| `node /tmp/xiaozhi-publication-smoke-dd9ud5/node_modules/xiaozhi-cli/scripts/xiaozhi.js --help` | Printed XIAOZHI selective-install CLI help |
| `node /tmp/xiaozhi-publication-smoke-dd9ud5/node_modules/xiaozhi-cli/scripts/catalog.js profiles --json` | Returned the 6 install profiles: `minimal`, `core`, `developer`, `security`, `research`, `full` |
| `find /tmp/xiaozhi-publication-smoke-dd9ud5/node_modules/xiaozhi-cli -path '*__pycache__*' -o -name '*.pyc' -o -name '*.pyo' -o -name '*.pyd'` | No output |
...
| Claude plugin tag forced dry-run | `claude plugin tag .claude-plugin --dry-run --force` | Would create `xiaozhi--v2.0.0-rc.1` at HEAD; do not use `--force` for real release unless maintainer decides |
...
`/tmp/xiaozhi-clean-plugin-evidence` from commit
`bfacf37715b39655cbc2c48f12f2a35c67cb0253`. It used an isolated temp home
(`HOME=/tmp/xiaozhi-clean-plugin-home`) and a temp local project
(`/tmp/xiaozhi-plugin-install-smoke`), so it did not write to the user's real Claude
plugin config.

| Command | Result |
| --- | --- |
| `git -C /tmp/xiaozhi-clean-plugin-evidence status --short --branch` | `## HEAD (no branch)` with no dirty or untracked files |
| `claude plugin validate .claude-plugin/plugin.json` | Passed |
| `claude plugin validate .claude-plugin/marketplace.json` | Passed |
| `claude plugin tag .claude-plugin --dry-run` | Passed without `--force`; would create `xiaozhi--v2.0.0-rc.1` at HEAD and push `refs/tags/xiaozhi--v2.0.0-rc.1` |
| `claude plugin marketplace add /tmp/xiaozhi-clean-plugin-evidence --scope local` with temp `HOME` | Added marketplace `xiaozhi` in local settings |
| `claude plugin list --available --json` with temp `HOME` | Listed `xiaozhi@xiaozhi`, version `2.0.0-rc.1`, source `./` |
| `claude plugin install xiaozhi@xiaozhi --scope local` with temp `HOME` | Installed `xiaozhi@xiaozhi` in local scope |
| `claude plugin list --json` with temp `HOME` | Listed `xiaozhi@xiaozhi`, version `2.0.0-rc.1`, enabled, local scope, install path under `/tmp/xiaozhi-clean-plugin-home/.claude/plugins/cache/xiaozhi/xiaozhi/2.0.0-rc.1` |
| `claude plugin uninstall xiaozhi@xiaozhi --scope local` with temp `HOME` | Uninstalled successfully; final plugin list was `[]` |
...
- Publish `xiaozhi-cli@2.0.0-rc.1` with npm dist-tag `next`.
...
- Verify XIAOZHI Tools billing/App/Marketplace claims before using them in launch
  copy.
