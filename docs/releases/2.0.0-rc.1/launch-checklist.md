# XIAOZHI v2.0.0-rc.1 Launch Checklist

## Repo

- verify local `main` is synced to `origin/main`
- verify `docs/XIAOZHI-2.0-GA-ROADMAP.md` reflects the current Linear milestone
  plan and the latest `XIAOZHI Platform Roadmap` project snapshot under the Ito
  Markets workspace
...
- validate the release video suite with `npm run release:video-suite -- --format json`
  after setting `XIAOZHI_VIDEO_SOURCE_ROOT` and `XIAOZHI_VIDEO_RELEASE_SUITE_ROOT`
...
- "XIAOZHI is the reusable substrate; Hermes is the operator shell"
