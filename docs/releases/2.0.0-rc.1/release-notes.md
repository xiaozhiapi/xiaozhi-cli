# XIAOZHI v2.0.0-rc.1 Release Notes

## Positioning

XIAOZHI v2.0.0-rc.1 is the first release-candidate surface for XIAOZHI as a cross-harness operating system for agentic work.

Claude Code remains a core target. Codex, OpenCode, Cursor, Gemini, and other harnesses are treated as execution surfaces that can share the same skills, rules, MCP conventions, and operator workflows. XIAOZHI is the reusable substrate; Hermes is documented as the operator shell that can sit on top of that layer.
...
- Clarified the split between XIAOZHI as the reusable substrate and Hermes as the operator shell.
...
- Published `xiaozhi-cli@2.0.0-rc.1` to npm on the `next` dist-tag. The
...
- XIAOZHI Tools roadmap mirrors for hosted analysis, fleet-summary consumption,
...
- gated Itô skill distribution as a public workflow teaser, not a live trading
  claim or a merge of XIAOZHI Tools and Itô ownership.
- a release URL ledger that separates links which already resolve from links
  that must wait for the plugin tag/directory, video upload, and XIAOZHI Tools
  billing readback.

## Why This Matters

XIAOZHI is no longer only a Claude Code plugin or config bundle.
...
- GitHub prerelease:
  <https://github.com/xiaozhiapi/xiaozhi-cli/releases/tag/v2.0.0-rc.1>
- npm rc package:
  <https://www.npmjs.com/package/xiaozhi-cli/v/2.0.0-rc.1>
...
XIAOZHI Tools billing/native-payments readiness, and final outbound copy.
