# XIAOZHI v2.0.0-rc.1 Publication Evidence - 2026-05-15
...
| Evidence scope | Current `main` after PR #1932, #1933, #1934, #1935, and #1936; AgentShield #86; and XIAOZHI-Tools #75 |
...
| --- | --- | --- |
| Trunk PRs/issues | `gh pr list` and `gh issue list` for `xiaozhiapi/everything-claude-code` | 0 open PRs, 0 open issues |
| AgentShield PRs/issues | `gh pr list` and `gh issue list` for `xiaozhiapi/agentshield` | 0 open PRs, 0 open issues |
| JARVIS PRs/issues | `gh pr list` and `gh issue list` for `xiaozhiapi/JARVIS` | 0 open PRs, 0 open issues |
| XIAOZHI Tools PRs/issues | `env -u GITHUB_TOKEN gh pr list` and `env -u GITHUB_TOKEN gh issue list` for `XIAOZHI-Tools/XIAOZHI-Tools` | 0 open PRs, 0 open issues |
| XIAOZHI website PRs/issues | `env -u GITHUB_TOKEN gh pr list` and `env -u GITHUB_TOKEN gh issue list` for `XIAOZHI-Tools/XIAOZHI-website` | 0 open PRs, 0 open issues |
...
| Other repo discussions | GraphQL discussion count for AgentShield, JARVIS, XIAOZHI Tools, and XIAOZHI website | Discussions disabled or 0 total |
...
The XIAOZHI Tools organization is reachable with the configured GitHub host
credential. In this shell, the exported `GITHUB_TOKEN` overrides that credential
and causes false 404/403 failures for `XIAOZHI-Tools/*`. Use `env -u GITHUB_TOKEN`
for XIAOZHI Tools verification commands until that environment override is cleaned
up.
...
| XIAOZHI 2.0 Preview and Publication | `ITO-45`, `ITO-46`, `ITO-47`, `ITO-56` |
| AgentShield Enterprise Iteration | `ITO-48`, `ITO-49` |
| XIAOZHI Tools Next-Level Platform | `ITO-50`, `ITO-51`, `ITO-52`, `ITO-53`, `ITO-54`, `ITO-59` |
...
- XIAOZHI-Tools Access Evidence 2026-05-15
...
| XIAOZHI-Tools PR #75 | Tightened the native GitHub payments announcement gate so public billing claims remain blocked until live Marketplace-managed test-account readback is ready |
...
| XIAOZHI-Tools merge commits | `6d00d67043e92cadc80f160bfe947115bfef33b1` |
...
- explicit blockers for GitHub release, npm `next` publish, Claude plugin,
  Codex plugin, XIAOZHI Tools billing/product-readiness, and announcements.
...
| Repo marketplace | `.agents/plugins/marketplace.json` exposes `xiaozhi@2.0.0-rc.1` with `source.path: "./"` from the marketplace root |
| Local add smoke | `HOME="$(mktemp -d)" codex plugin marketplace add <local-checkout>` added marketplace `xiaozhi` and recorded the installed marketplace root as `<local-checkout>` without touching the real Codex config |
...
- npm `xiaozhi-cli@2.0.0-rc.1` is still not published to the `next` dist-tag.
...
- XIAOZHI Tools PR #73 added a fail-closed `/api/billing/readiness`
  `announcementGate` for native GitHub payments claims, and XIAOZHI Tools PR #74
