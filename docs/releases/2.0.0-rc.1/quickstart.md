# XIAOZHI v2.0.0-rc.1 Quickstart
...
```bash
npm install xiaozhi-cli@next
```

`next` currently resolves to `xiaozhi-cli@2.0.0-rc.1`; `latest` remains on
...
It shows the intended XIAOZHI 2.0 pattern:
...
- Claude Code consumes XIAOZHI through the Claude plugin and native hooks.
- Codex consumes XIAOZHI through `AGENTS.md`, `.codex-plugin/plugin.json`, and MCP reference config.
- OpenCode consumes XIAOZHI through the OpenCode package/plugin surface.
