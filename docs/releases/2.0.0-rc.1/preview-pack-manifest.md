# XIAOZHI v2.0.0-rc.1 Preview Pack Manifest
...
| `skills/hermes-imports/SKILL.md` | Sanitized Hermes-to-XIAOZHI import workflow | Includes import rules, sanitization checklist, conversion pattern, and output contract |
...
| `docs/releases/2.0.0-rc.1/naming-and-publication-matrix.md` | Naming, slug, and publication-path decision record | Keeps `XIAOZHI`, npm `xiaozhi-cli`, and plugin slug `xiaozhi` for rc.1 |
...
| `docs/releases/2.0.0-rc.1/ito-prediction-market-skill-pack.md` | Public Itô skill-pack distribution note | Keeps Itô API access gated, non-advisory, and separate from XIAOZHI Tools billing |
...
operator patterns feeding back into XIAOZHI without merging business ownership.
...
place trades, do not provide investment advice, and do not merge XIAOZHI Tools with
...
skills should enter XIAOZHI only after they pass the same rules:
...
| Reference pressure | XIAOZHI preview-pack interpretation |
| --- | --- |
...
- npm `xiaozhi-cli@2.0.0-rc.1` on the `next` dist-tag live readback;
- Claude plugin tag / marketplace propagation for `xiaozhi@xiaozhi`;
...
- XIAOZHI Tools billing/product readiness evidence remains fresh: the May 20
