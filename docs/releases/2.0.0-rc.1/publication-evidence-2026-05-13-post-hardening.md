# XIAOZHI v2.0.0-rc.1 Publication Evidence - 2026-05-13 Post-Hardening
...
| --- | --- | --- |
| GitHub PRs and issues | `gh pr list` / `gh issue list` across trunk, AgentShield, and JARVIS | 0 open PRs and 0 open issues on accessible `xiaozhiapi` repos |
| Trunk discussions | GraphQL discussion count for `xiaozhiapi/everything-claude-code` | 0 open discussions |
...
XIAOZHI-Tools organization repo counts were not rechecked through the current
GraphQL token in this pass because the token cannot resolve those org repos.
The prior post-#42 local checkout handoff recorded both XIAOZHI-Tools repos at
...
| Rust advisory audit | `cd xiaozhi2 && cargo audit -q` | Passed silently |
...
The May 2026 TanStack incident maps to XIAOZHI release risk through three workflow
classes:
...
XIAOZHI's current guardrails cover those classes through:
