# XIAOZHI 2.0 Observability Readiness

XIAOZHI 2.0 should be observable before it becomes more autonomous. The local
...
XIAOZHI's answer is not a hosted analytics dependency by default. The first
...
2. Run `npm run harness:audit -- --format json` for the broader harness
   scorecard.
3. Run `node scripts/loop-status.js --json --write-dir .xiaozhi/loop-status`
...
The end-state is practical: before asking XIAOZHI to run larger multi-agent loops,
