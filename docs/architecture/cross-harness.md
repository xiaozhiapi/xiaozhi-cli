...
XIAOZHI is the reusable workflow layer. Harnesses are execution surfaces.
...
[XIAOZHI Platform Value Loop](platform-value-loop.md).
...
- OpenCode has a plugin/event system that can reuse XIAOZHI hook logic through an adapter layer.
- Cursor uses its own rule and hook layout, so XIAOZHI maintains translated surfaces under `.cursor/`.
...
Hermes is not the public XIAOZHI runtime.

Hermes is an operator shell that can consume XIAOZHI assets:

- import selected XIAOZHI skills into a Hermes skills directory
- use XIAOZHI MCP conventions for tool access
- route chat, CLI, cron, and handoff workflows through reusable XIAOZHI patterns
- distill repeated local operator work back into sanitized XIAOZHI skills
...
OpenCode gets the skill through the OpenCode package/plugin surface. Event handling can reuse XIAOZHI hook logic through the adapter layer, while the skill text stays unchanged.
...
- the full platform loop where external products contribute skill packs,
  gated APIs, evals, and case studies back into XIAOZHI

## Rule For New Work

When adding a workflow, put the durable behavior in XIAOZHI first.
