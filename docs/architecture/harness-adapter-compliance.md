...
This matrix is the public onramp for teams that want to use XIAOZHI across more
...
XIAOZHI's durable units stay in shared sources:
...
| State | Meaning |
| --- | --- |
| Native | XIAOZHI can install or verify the surface directly for this harness. |
| Adapter-backed | XIAOZHI has a thin adapter, plugin, or package surface, but parity differs by harness. |
| Instruction-backed | XIAOZHI can provide the guidance and files, but the harness does not expose the runtime hook/session surface XIAOZHI needs for enforcement. |
| Reference-only | The tool is useful as a design pressure or external runtime, but XIAOZHI does not yet ship a direct installer or adapter for it. |
...
| Gemini | Instruction-backed | Gemini project-local instructions; shared skills; rules; compatibility docs | No full XIAOZHI hook parity; ecosystem ports must document drift from upstream XIAOZHI | `./install.sh --profile minimal --target gemini` | `node tests/lib/install-targets.test.js` | Treat Gemini ports as ecosystem adapters until validated end to end inside Gemini CLI. |
...
| Orca | Reference-only | worktree lifecycle; review state; notification; provider-identity design pressure | No XIAOZHI installer or direct adapter today | Use as a comparison target for worktree/session state requirements | `npm run observability:ready` | Do not import product-specific assumptions; convert lessons into XIAOZHI event fields. |
| Superset | Reference-only | workspace presets; parallel-agent review loops; worktree isolation design pressure | No XIAOZHI installer or direct adapter today | Use as a comparison target for workspace preset taxonomy | `npm run observability:ready` | Keep XIAOZHI portable; do not require a desktop workspace to get basic value. |
| Ghast | Reference-only | terminal-native pane grouping; cwd grouping; search; notifications | No XIAOZHI installer or direct adapter today | Use as a comparison target for terminal-first session grouping | `node scripts/session-inspect.js --list-adapters` | Preserve terminal ergonomics before adding visual UI assumptions. |
...
Use this sequence before asking XIAOZHI to make a team or repo setup more
autonomous:
...
node scripts/loop-status.js --json --write-dir .xiaozhi/loop-status
```
...
- Treat reference-only tools as design pressure until XIAOZHI has a direct adapter.
