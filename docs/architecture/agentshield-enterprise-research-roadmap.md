...
- XIAOZHI-Tools now consumes that fleet primitive in hosted security review:
...
default. XIAOZHI-Tools follow-up routing now consumes fleet summaries and surfaces
...
AgentShield is already connected conceptually to the XIAOZHI Tools GitHub App.
...
- Billing readiness checks shared with XIAOZHI-Tools so payment state never changes
...
- It aligns AgentShield with XIAOZHI's harness-agnostic positioning.
...
Connect AgentShield findings to XIAOZHI-Tools follow-up routing.

Artifacts:

- Finding fingerprints compatible with XIAOZHI-Tools issue caps.
...
- XIAOZHI-Tools can consume the finding fingerprints or backlog export without
