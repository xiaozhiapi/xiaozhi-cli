...
- The next open XIAOZHI Phase 1 items are:
  - review `#399`
  - convert recurring discussion pressure into tracked issues
  - define selective-install architecture
  - write the XIAOZHI 2.0 discovery doc
- `agentshield`, `XIAOZHI-website`, and `skill-creator-app` all have dirty
...
- `applications/` is not a standalone git repo. It lives inside the parent
  workspace repo at `<XIAOZHI_ROOT>`.
...
```text
Work in: <XIAOZHI_ROOT>/everything-claude-code
...
issues. Focus on high-signal roadmap items that unblock XIAOZHI 1.x and XIAOZHI 2.0.
...
5. XIAOZHI 2.0 discovery doc / adapter contracts
...
### Prompt C — XIAOZHI 2.0 Discovery and Adapter Spec

```text
Work in: <XIAOZHI_ROOT>/everything-claude-code

Goal:
Turn the existing XIAOZHI 2.0 vision into a first concrete discovery doc focused on
...
4. Define what stays in XIAOZHI 1.x versus what belongs in XIAOZHI 2.0.
...
```text
Work in: <XIAOZHI_ROOT>/agentshield
...
## Repo: `XIAOZHI-website`

### Prompt — Landing Rewrite and Product Framing

```text
Work in: <XIAOZHI_ROOT>/XIAOZHI-website
...
- distinguish XIAOZHI 1.x toolkit from XIAOZHI 2.0 control plane clearly
...
```text
Work in: <XIAOZHI_ROOT>/skill-creator-app
...
1. Assess whether the app should support:
   ...
   - XIAOZHI convention adaptation workflows
...
- keep the focus on audited skill ingestion and XIAOZHI-compatible output
...
## Repo: `XIAOZHI` Workspace (`applications/`, `knowledge/`, `tasks/`)

### Prompt — Example Apps and Workflow Reliability Proofs

```text
Work in: <XIAOZHI_ROOT>

Goal:
Use the parent XIAOZHI workspace to support the mega-plan hosted/workflow lanes.
...
- keep recommendations compatible with the current multi-repo XIAOZHI setup
...
The current worktree should stay on XIAOZHI-native Phase 1 work that does not touch
the existing dirty skill-file changes here. The best next local tasks are:

1. selective-install architecture
2. XIAOZHI 2.0 discovery doc
3. PR `#399` review
