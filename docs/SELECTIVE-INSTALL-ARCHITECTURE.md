...
- what XIAOZHI owns and may safely remove or repair later

That is the missing contract between XIAOZHI 1.x installation and an XIAOZHI 2.0
control plane.
...
- unified `xiaozhi` CLI routing install, planning, and lifecycle commands
...
- legacy `xiaozhi-install` compatibility still points at `install.sh`
...
- the repo already has a unified Node entrypoint through `xiaozhi` and
  `install-apply.js`
...
XIAOZHI 2.0 should make operation planning a standalone phase so that:
...
- `uninstall`: remove only XIAOZHI-owned outputs
...
- `scripts/xiaozhi.js` as the unified CLI
...
If the goal is XIAOZHI 2.0 and not just “working enough,” the next modularization
steps should be:
...
Today XIAOZHI still behaves like a broad payload copier:
...
That means XIAOZHI 2.0 needs two contracts, not one:
...
Fields still needed for XIAOZHI 2.0:

- `installStrategy`
  for example `copy`, `flatten-rules`, `generate`, `merge-config`
- `ownership`
  whether XIAOZHI fully owns the target path or only generated files under it
- `pathMode`
...
That lets XIAOZHI 2.0 say things like:
...
XIAOZHI 2.0 needs the next layer: operation planning.
...
```json
{
  ...
  "destination": "/Users/example/.claude/rules/xiaozhi/common/coding-style.md",
  "ownership": "managed",
...
Install-state is the durable contract that XIAOZHI 1.x is missing.

Suggested path conventions:

- Claude target:
  `~/.claude/xiaozhi/install-state.json`
- Cursor target:
  `./.cursor/xiaozhi-install-state.json`
- Antigravity target:
  `./.agent/xiaozhi-install-state.json`
- future Codex target:
  `~/.codex/xiaozhi-install-state.json`
...
- enough detail for uninstall to remove only XIAOZHI-managed outputs
...
- `xiaozhi list-installed` routes to `node scripts/list-installed.js`
- `xiaozhi uninstall` routes to `node scripts/uninstall.js`
- `xiaozhi doctor` routes to `node scripts/doctor.js`
- `xiaozhi repair` routes to `node scripts/repair.js`
...
- load install-state
- remove only XIAOZHI-managed destinations recorded in state
...
XIAOZHI 2.0 should translate legacy language arguments into a compatibility request.
...
XIAOZHI 2.0 should improve this carefully.
...
3. define `xiaozhi.install.v1` state schema
...
4. decide when `xiaozhi-install` should become a thin alias for `xiaozhi install`
...
3. reduce package shipping surface to the module graph instead of broad folders
4. decide whether `xiaozhi-install` remains separate or becomes `xiaozhi install`
5. add tests that lock down:
   - target-specific merge/remove behavior
   - repair and uninstall safety for non-copy operations
   - unified `xiaozhi` CLI routing and compatibility guarantees
...
That is the shortest path from XIAOZHI 1.x installer sprawl to an XIAOZHI 2.0
