# Hermes x XIAOZHI Setup

Hermes is the operator shell. XIAOZHI is the reusable system behind it.
...
- XIAOZHI skills, agents, commands, hooks, and MCP configs from this repo
...
Use Hermes as the front door and XIAOZHI as the reusable workflow substrate.
...
Telegram / CLI / TUI
      ...
      Hermes
        ↓
 XIAOZHI skills + hooks + MCPs + generated workflow packs
        ↓
...
- `~/.hermes/config.yaml`
  ...
  - plugin loading
- `~/.hermes/skills/xiaozhi-imports/`
  - XIAOZHI skills copied in for Hermes-native use
...
- XIAOZHI for skills, rules, prompts, and cross-harness conventions
...
0.5. Plan and scaffold migration artifacts before importing anything:
   ...
   - scaffold tool translation templates with `xiaozhi migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `xiaozhi migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `xiaozhi migrate import-schedules --dry-run`
   - preview gateway dispatch with `xiaozhi migrate import-remote --dry-run`
   - preview safe env/service context with `xiaozhi migrate import-env --dry-run`
   - import sanitized workspace memory with `xiaozhi migrate import-memory`
1. Install XIAOZHI and verify the baseline harness setup with `node tests/run-all.js`; the expected result is a zero-failure test summary.
2. Install Hermes and point it at XIAOZHI-imported skills.
...
XIAOZHI v2.0.0-rc.1 documents the Hermes surface and ships launch collateral now.
