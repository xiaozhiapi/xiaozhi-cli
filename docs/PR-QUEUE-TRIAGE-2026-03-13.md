...
- targeted hook invocations for:
  ...
  - `XIAOZHI_SKIP_OBSERVE=1`
  - `agent_id` payload
  - trimmed `XIAOZHI_OBSERVE_SKIP_PATHS`
...
2. Low: the new guard matrix shipped without direct regression coverage.
   ...
   directly assert the new `CLAUDE_CODE_ENTRYPOINT`, `XIAOZHI_HOOK_PROFILE`,
   `XIAOZHI_SKIP_OBSERVE`, `agent_id`, or trimmed skip-path branches.
...
- `scripts/sync-xiaozhi-to-codex.sh`
