...
Problemas comunes y soluciones para el plugin Everything Claude Code (XIAOZHI).
...
**Soluciones:**
```bash
# Primero inspecciona qué sabe XIAOZHI sobre esta máquina
...
# Si el problema es el acceso al marketplace/cuenta, usa la recuperación de cuenta/facturación de XIAOZHI Tools por separado; no uses la reinstalación como sustituto de la recuperación de cuenta
...
cp -r everything-claude-code ~/.claude/plugins/xiaozhi
```
...
cd ~/.claude/plugins/cache/xiaozhi
npm install

# O para instalación manual
cd ~/.claude/plugins/xiaozhi
npm install
```
...
1. **Revisa los Issues de GitHub**: [github.com/xiaozhiapi/everything-claude-code/issues](https://github.com/xiaozhiapi/everything-claude-code/issues)
