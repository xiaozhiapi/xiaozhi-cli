...
Use this guide when you want XIAOZHI behavior inside a harness that does not natively load `.claude/`, `.codex/`, `.opencode/`, `.cursor/`, or `.agent/` layouts.
...
Prefer a first-class XIAOZHI target whenever one exists:
...
Use this guide only when you need XIAOZHI behavior in a non-native harness.

## What You Are Reproducing

When you adapt XIAOZHI manually, you are trying to preserve four things:
...
sed -n '1,220p' skills/tdd-workflow/SKILL.md > /tmp/xiaozhi-context.md
printf '\n\n---\n\n' >> /tmp/xiaozhi-context.md
sed -n '1,220p' skills/backend-patterns/SKILL.md >> /tmp/xiaozhi-context.md
printf '\n\n---\n\n' >> /tmp/xiaozhi-context.md
sed -n '1,220p' skills/security-review/SKILL.md >> /tmp/xiaozhi-context.md
```
...
Optional: if you already use a repo packer like `repomix`, it can help compress selected files into one handoff document. It is a convenience tool, not the canonical XIAOZHI path.

## Compression Rules

When manually packing XIAOZHI for another harness:
...
You are not implementing real commands. You are giving the harness explicit invocation handles that map to XIAOZHI behavior.
...
That does not recreate true automation, but it captures the operational discipline of XIAOZHI.

## Harness Capability Matrix

| Capability | First-Class XIAOZHI Targets | Manual-Adaptation Targets |
| --- | --- | --- |
...
2. Pack the selected XIAOZHI skill files into one upload or paste block.
...
```text
You are operating with a manually adapted XIAOZHI bundle.
...
- use manual adaptation to carry XIAOZHI behavior into non-native harnesses
- use native XIAOZHI targets whenever you want the full system

## Related Work

- [Issue #1186](https://github.com/xiaozhiapi/everything-claude-code/issues/1186)
- [Discussion #1077](https://github.com/xiaozhiapi/everything-claude-code/discussions/1077)
