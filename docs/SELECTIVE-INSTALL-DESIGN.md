# XIAOZHI Selective Install Design

## Purpose

This document defines the user-facing selective-install design for XIAOZHI.
...
- how users choose XIAOZHI components
- what the CLI should feel like
- what config file should exist
- how installation should behave across harness targets
- how the design maps onto the current XIAOZHI codebase without requiring a rewrite

## Problem

Today XIAOZHI still feels like a large payload installer even though the repo now
...
The selective-install system should make XIAOZHI feel composable instead of
all-or-nothing.
...
1. Let users install a small default XIAOZHI footprint quickly.
...
5. Preserve backward compatibility with the current `xiaozhi-install typescript`
   style during rollout.

## Non-Goals

- packaging XIAOZHI into multiple npm packages in the first phase
...
A user should be able to get a useful XIAOZHI install with one command:

```bash
xiaozhi install --target claude --profile core
```
...
```bash
xiaozhi install --target cursor --profile developer --with lang:typescript --with framework:nextjs --dry-run
```
...
```bash
xiaozhi install --config xiaozhi-install.json
```
...
These are the default XIAOZHI building blocks:
...
Capability packs are cross-cutting XIAOZHI feature bundles.
...
- `core`
  minimal baseline, safe default for most users trying XIAOZHI
- `developer`
...
```bash
xiaozhi install --target claude --profile developer --with lang:typescript --with framework:nextjs --without capability:orchestration
```
...
```bash
xiaozhi install [--target <target>] [--profile <name>] [--with <component>]... [--without <component>]... [--config <path>] [--dry-run] [--json]
```
...
```bash
xiaozhi install --target claude --profile core
xiaozhi install --target cursor --profile developer --with lang:typescript --with framework:nextjs
xiaozhi install --target antigravity --with capability:security --with lang:python
xiaozhi install --config xiaozhi-install.json
```
...
```bash
xiaozhi plan [same selection flags as install]
```
...
```bash
xiaozhi-install typescript
xiaozhi-install --target cursor typescript
xiaozhi typescript
```
...
```json
{
  "$schema": "./schemas/xiaozhi-install-config.schema.json",
  "version": 1,
  "target": "cursor",
  "profile": "developer",
  "include": [
...
The commands differ in action, not in how XIAOZHI understands the selected install.
...
- `xiaozhi-install.json` config file parsing
...
## Mapping To Current XIAOZHI Manifests
...
That is the shortest path from the current XIAOZHI codebase to a real selective
install experience that feels like XIAOZHI 2.0 instead of a large legacy installer.
