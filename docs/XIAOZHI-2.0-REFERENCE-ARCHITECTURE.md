# XIAOZHI 2.0 Reference Architecture

Current execution mirror:
[`XIAOZHI-2.0-GA-ROADMAP.md`](XIAOZHI-2.0-GA-ROADMAP.md).

This document turns the May 2026 reference sweep into concrete XIAOZHI backlog
...
| [`stanford-iris-lab/meta-harness`](https://github.com/stanford-iris-lab/meta-harness) | Automated search over task-specific harness design: what to store, retrieve, and show. | Split XIAOZHI improvement loops into scenario spec, proposer trace, verifier result, and promoted playbook. |
...
| [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent) | Self-improving operator shell with memories, skills, scheduler, gateways, subagents, terminal backends, and migration tooling. | Keep XIAOZHI portable across local, SSH, container, and hosted terminal backends without hiding the underlying commands. |
...
XIAOZHI 2.0 should be a harness operating system, not only a catalog of commands,
agents, and skills.
...
│ AgentShield policies/SARIF, XIAOZHI Tools checks, billing,       │
...
- Use RAG/reference sets for vetted XIAOZHI patterns, team history, CI failures,
...
- `XIAOZHI-Tools`: map analyzer findings to PR comments, check runs, and Linear
...
### XIAOZHI Tools Commercial And Review Platform

XIAOZHI Tools should become the GitHub-native layer for billing, deep analysis,
...
- Deep analyzer comparable in scope to the useful parts of GitGuardian,
  ...
  and harness drift.
- RAG/reference set over vetted XIAOZHI patterns, historical PR outcomes,
...
5. Audit XIAOZHI Tools billing and check-run surfaces.
