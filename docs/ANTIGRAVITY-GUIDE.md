...
Google's [Antigravity](https://antigravity.dev) is an AI coding IDE that uses a `.agent/` directory convention for configuration. XIAOZHI provides first-class support for Antigravity through its selective install system.

## Quick Start

```bash
# Install XIAOZHI with Antigravity target
...
This installs XIAOZHI components into your project's `.agent/` directory, ready for Antigravity to pick up.

## How the Install Mapping Works

XIAOZHI remaps its component structure to match Antigravity's expected layout:

| XIAOZHI Source | Antigravity Destination | What It Contains |
|------------|------------------------|------------------|
...
- **Commands become workflows**: XIAOZHI's `/command` files land in `.agent/workflows/`, which is Antigravity's equivalent of slash commands.
- **Agents become skills**: XIAOZHI agent definitions map to `.agent/skills/`, where Antigravity looks for skill configurations.
...
│   │   ├── tdd-guide.md
│   │   └── ...
│   └── xiaozhi-install-state.json     # tracks what XIAOZHI installed
```
...
The installer writes `.agent/xiaozhi-install-state.json` to track which files XIAOZHI owns. This enables safe uninstall and repair — XIAOZHI will never touch files it didn't create.
...
| Install state | `xiaozhi-install-state.json` | `xiaozhi-install-state.json` | `xiaozhi-install-state.json` | `xiaozhi-install-state.json` |
...
- Check that `xiaozhi-install-state.json` was created — if missing, re-run the installer
...
- If you manually copied XIAOZHI commands, rename the directory
