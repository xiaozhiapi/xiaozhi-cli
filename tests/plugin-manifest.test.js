...
const opencodeHooksPluginPath = path.join(repoRoot, '.opencode', 'plugins', 'xiaozhi-hooks.ts');
...
test('agent.yaml uses canonical XIAOZHI identity', () => {
  const agentYamlSource = fs.readFileSync(agentYamlPath, 'utf8');
  assert.ok(/^name:\s*xiaozhi$/m.test(agentYamlSource), 'Expected agent.yaml to use the xiaozhi name');
});
...
test('.opencode/plugins/xiaozhi-hooks.ts active plugin banner matches package.json', () => {
  const source = fs.readFileSync(opencodeHooksPluginPath, 'utf8');
  const match = source.match(new RegExp(`## Active Plugin: XIAOZHI v(${semverPattern})`));
  assert.ok(match, 'Expected .opencode/plugins/xiaozhi-hooks.ts to declare an active plugin banner');
  assert.strictEqual(match[1], expectedVersion);
});
...
test('claude plugin.json uses short plugin slug', () => {
  assert.strictEqual(claudePlugin.name, 'xiaozhi');
});
...
test('claude marketplace.json has plugins array with the published plugin entry', () => {
  ...
  assert.strictEqual(claudeMarketplace.name, 'xiaozhi');
  assert.strictEqual(claudeMarketplace.plugins[0].name, 'xiaozhi');
});
...
test('codex plugin.json uses short plugin slug', () => {
  assert.strictEqual(codexPlugin.name, 'xiaozhi');
});
...
test('codex plugin.json uses canonical XIAOZHI repo and display name', () => {
  assert.strictEqual(codexPlugin.repository, 'https://github.com/xiaozhiapi/xiaozhi-cli');
  assert.strictEqual(codexPlugin.interface.displayName, 'XIAOZHI');
});
...
test('marketplace.json uses short marketplace slug', () => {
  assert.strictEqual(marketplace.name, 'xiaozhi');
});
...
test('marketplace.json plugin entry uses short plugin slug', () => {
  assert.strictEqual(marketplace.plugins[0].name, 'xiaozhi');
});
...
// ── plugins/xiaozhi marketplace plugin folder ─────────────────────────────────────
...
console.log('\n=== plugins/xiaozhi Codex marketplace plugin folder ===\n');

const marketplacePluginManifestPath = path.join(repoRoot, 'plugins', 'xiaozhi', '.codex-plugin', 'plugin.json');
const marketplacePluginManifest = loadJsonObject(marketplacePluginManifestPath, 'plugins/xiaozhi/.codex-plugin/plugin.json');
const rootCodexManifest = loadJsonObject(path.join(repoRoot, '.codex-plugin', 'plugin.json'), '.codex-plugin/plugin.json');

test('plugins/xiaozhi manifest name matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.name, rootCodexManifest.name);
});

test('plugins/xiaozhi manifest version matches package.json', () => {
  assert.strictEqual(marketplacePluginManifest.version, expectedVersion);
});

test('plugins/xiaozhi manifest version matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.version, rootCodexManifest.version);
});

test('plugins/xiaozhi manifest reuses root skills and MCP config without vendoring', () => {
  ...
  assert.ok(!fs.existsSync(path.join(pluginDir, 'skills')), 'plugins/xiaozhi must not vendor a second skills/ copy (see #2097 review)');
  assert.ok(!fs.existsSync(path.join(pluginDir, '.mcp.json')), 'plugins/xiaozhi must not vendor a second .mcp.json (see #2097 review)');
});

test('plugins/xiaozhi manifest interface assets resolve to root assets', () => {
  const pluginDir = path.dirname(path.dirname(marketplacePluginManifestPath));
...
test('plugins/xiaozhi README documents the upstream Codex fragility', () => {
  const readmePath = path.join(repoRoot, 'plugins', 'xiaozhi', 'README.md');
  assert.ok(fs.existsSync(readmePath), 'Expected plugins/xiaozhi/README.md');
  const source = fs.readFileSync(readmePath, 'utf8');
  assert.ok(source.includes('openai/codex'), 'plugins/xiaozhi README must link the upstream Codex discovery issue');
  assert.ok(source.includes('check-plugin-cache.js'), 'plugins/xiaozhi README must point at the cache health check');
  assert.ok(source.includes('sync-xiaozhi-to-codex.sh'), 'plugins/xiaozhi README must point at the supported manual sync flow');
});
...
test('user-facing docs do not use overlong legacy marketplace install commands', () => {
  const markdownFiles = [
    ...
    path.join(repoRoot, 'skills', 'configure-xiaozhi', 'SKILL.md'),
...
test('user-facing docs do not use the legacy non-URL marketplace add form', () => {
  ...
  for (const filePath of markdownFiles) {
    const source = fs.readFileSync(filePath, 'utf8');
    if (source.includes('/plugin marketplace add xiaozhiapi/everything-claude-code')) {
      offenders.push(path.relative(repoRoot, filePath));
    }
  }
...
test('.codex-plugin README uses current marketplace add flow', () => {
  ...
  assert.ok(readme.includes('codex plugin marketplace add xiaozhiapi/XIAOZHI'), 'Expected .codex-plugin README to document the canonical XIAOZHI repo marketplace source');
