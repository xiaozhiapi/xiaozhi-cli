...
async function loadPlugin() {
  ...
  const pluginUrl = pathToFileURL(
    path.join(repoRoot, ".opencode", "dist", "plugins", "xiaozhi-hooks.js")
  ).href
...
async function withTempProject(files, fn) {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "xiaozhi-opencode-plugin-"))
  try {
...
async function main() {
  ...
  const { XiaozhiHooksPlugin } = await loadPlugin()
  const tests = [
    [
      ...
      async () => withTempProject(
        ...
        async (projectDir) => {
          ...
          const hooks = await XiaozhiHooksPlugin({ client, $, directory: projectDir })
          ...
          // Verify XIAOZHI_VERSION is not hardcoded
          assert.ok(env.XIAOZHI_VERSION !== "1.8.0", "XIAOZHI_VERSION should not be hardcoded to 1.8.0")
          assert.ok(env.XIAOZHI_VERSION.match(/^\d+\.\d+\.\d+$/), "XIAOZHI_VERSION should be a valid semver version")
        }
      ),
    ],
    [
      ...
      async () => withTempProject(["CLAUDE.md"], async (projectDir) => {
        ...
        assert.deepStrictEqual($.calls, [], `Unexpected shell probes: ${$.calls.join(", ")}`)
        assert.ok(
          client.logs.some((entry) => entry.message === "[XIAOZHI] Found CLAUDE.md - loading project context"),
