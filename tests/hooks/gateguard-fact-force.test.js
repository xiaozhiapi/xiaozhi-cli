...
function runTests() {
  ...
  // --- Test 9: respects XIAOZHI_DISABLED_HOOKS ---
  clearState();
  if (test('respects XIAOZHI_DISABLED_HOOKS (skips when disabled)', () => {
    const input = {
      tool_name: 'Edit',
      tool_input: { file_path: '/src/disabled.js', old_string: 'a', new_string: 'b' }
    };
    const result = runHook(input, {
      XIAOZHI_DISABLED_HOOKS: 'pre:edit-write:gateguard-fact-force'
    });
  ...
  clearState();
  if (test('respects XIAOZHI_GATEGUARD=off without writing gate state', () => {
    const input = {
      tool_name: 'Write',
      tool_input: { file_path: '/src/env-disabled.js', content: 'export const ok = true;' }
    };
    const result = runHook(input, { XIAOZHI_GATEGUARD: 'off' });
    const output = parseOutput(result.stdout);
  ...
  if (test('denial messages include direct recovery escape hatch', () => {
    ...
    assert.strictEqual(output.hookSpecificOutput.permissionDecision, 'deny');
    assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('XIAOZHI_GATEGUARD=off'),
      'denial reason should show the direct recovery env toggle');
    assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('XIAOZHI_DISABLED_HOOKS'),
  ...
  function runFreshSessionEdit(filePath, extra = {}) {
    return runHook({
      ...
      ...extra
    }, { CLAUDE_SESSION_ID: '', XIAOZHI_SESSION_ID: '' });
  }
  ...
  if (test('emits a condensed single-line denial once the full-block budget is spent', () => {
    ...
    assert.ok(reason.includes('XIAOZHI_GATEGUARD=off'), 'condensed message keeps a recovery hint');
  })) passed++; else failed++;
