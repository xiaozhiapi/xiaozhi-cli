/**
 * Tests for scripts/hooks/xiaozhi-metrics-bridge.js
 *
 * Run with: node tests/hooks/xiaozhi-metrics-bridge.test.js
 */
...
const { run, hashToolCall, extractFilePaths, readSessionCost } = require('../../scripts/hooks/xiaozhi-metrics-bridge');
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-metrics-bridge-test-'));
}

function runTests() {
  console.log('\n=== Testing xiaozhi-metrics-bridge.js ===\n');
  ...
  if (
    test('readSessionCost suppresses repeated malformed breadcrumbs across hook subprocesses', () => {
      ...
      try {
        ...
        const bridgePath = path.resolve(__dirname, '../../scripts/hooks/xiaozhi-metrics-bridge');
