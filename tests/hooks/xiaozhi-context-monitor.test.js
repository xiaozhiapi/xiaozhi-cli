/**
 * Tests for scripts/hooks/xiaozhi-context-monitor.js
 *
 * Run with: node tests/hooks/xiaozhi-context-monitor.test.js
 */
...
const { run, evaluateConditions, detectLoop, severityLabel, costWarningsEnabled } = require('../../scripts/hooks/xiaozhi-context-monitor');
...
function runTests() {
  console.log('\n=== Testing xiaozhi-context-monitor.js ===\n');
  ...
  if (
    test('XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS=off disables only run-time cost warnings', () => {
      const sessionId = `ctx-monitor-cost-off-${process.pid}-${Date.now()}`;
      const input = JSON.stringify({ session_id: sessionId, tool_name: 'Bash' });
      const warnPath = path.join(os.tmpdir(), `xiaozhi-ctx-warn-${sessionId}.json`);
      try {
        ...
        const result = withEnv('XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS', 'off', () => JSON.parse(run(input)));
  ...
  if (
    test('cost warning env defaults on and accepts false-like values', () => {
      assert.strictEqual(withEnv('XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS', undefined, () => costWarningsEnabled()), true);
      assert.strictEqual(withEnv('XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS', 'false', () => costWarningsEnabled()), false);
      assert.strictEqual(withEnv('XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS', '0', () => costWarningsEnabled()), false);
      assert.strictEqual(withEnv('XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS', 'yes', () => costWarningsEnabled()), true);
    })
  )
    passed++;
  else failed++;
