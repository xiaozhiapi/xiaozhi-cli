...
function run(args = [], options = {}) {
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      XIAOZHI_PLUGIN_ROOT: options.eccRoot || '',
      ...(options.env || {}),
    },
...
function runTests() {
  ...
  if (test('normalizes Windows Git Bash POSIX drive roots', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/xiaozhi', 'win32'),
      'C:/Users/x/.claude/plugins/xiaozhi'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/z/Work/XIAOZHI/scripts/hooks/check-console-log.js', 'win32'),
      'Z:/Work/XIAOZHI/scripts/hooks/check-console-log.js'
    );
  })) passed++; else failed++;
  ...
  if (test('leaves already-Windows roots unchanged', () => {
    ...
    assert.strictEqual(
      normalizePluginRootForPlatform('D:\\Users\\x\\.claude\\plugins\\xiaozhi', 'win32'),
      'D:\\Users\\x\\.claude\\plugins\\xiaozhi'
    );
  })) passed++; else failed++;

  if (test('leaves POSIX-looking roots unchanged off Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/xiaozhi', 'darwin'),
      '/c/Users/x/.claude/plugins/xiaozhi'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/xiaozhi', 'linux'),
      '/c/Users/x/.claude/plugins/xiaozhi'
    );
  })) passed++; else failed++;
  ...
  if (test('does not mangle UNC or non-drive absolute paths on Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('\\\\server\\share\\xiaozhi', 'win32'),
      '\\\\server\\share\\xiaozhi'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/workspace/xiaozhi', 'win32'),
      '/workspace/xiaozhi'
    );
  })) passed++; else failed++;
...
process.stdout.write(JSON.stringify({
  ...
  eccRoot: process.env.XIAOZHI_PLUGIN_ROOT,
}));
`);
