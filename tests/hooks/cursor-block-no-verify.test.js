...
function runWrapper(input, env = {}) {
  ...
  const result = spawnSync('node', [wrapper], {
    input: rawInput,
    encoding: 'utf8',
    env: { ...process.env, XIAOZHI_HOOK_PROFILE: 'standard', ...env },
    timeout: 15000,
...
// --- Disable via XIAOZHI_DISABLED_HOOKS ---

if (test('respects XIAOZHI_DISABLED_HOOKS=pre:bash:block-no-verify', () => {
  const r = runWrapper(
    { command: 'git commit --no-verify -m "msg"' },
    { XIAOZHI_DISABLED_HOOKS: 'pre:bash:block-no-verify' }
  );
