...
function runTests() {
  ...
  if (test('pre dispatcher blocks --no-verify before other Bash checks', () => {
    ...
    const result = runScript(preDispatcher, input, { XIAOZHI_HOOK_PROFILE: 'strict' });
  ...
  if (test('pre dispatcher emits no stdout for a plain command (regression: issue #2239)', () => {
    ...
    const result = runScript(preDispatcher, input, { XIAOZHI_HOOK_PROFILE: 'standard' });
  ...
  if (test('pre dispatcher still honors per-hook disable flags', () => {
    ...
    const enabled = runScript(preDispatcher, input, { XIAOZHI_HOOK_PROFILE: 'strict' });
    ...
    const disabled = runScript(preDispatcher, input, {
      XIAOZHI_HOOK_PROFILE: 'strict',
      XIAOZHI_DISABLED_HOOKS: 'pre:bash:git-push-reminder',
    });
  ...
  if (test('pre dispatcher respects hook profiles inside the consolidated path', () => {
    const input = { tool_input: { command: 'git push origin main' } };
    const result = runScript(preDispatcher, input, { XIAOZHI_HOOK_PROFILE: 'minimal' });
    assert.strictEqual(result.status, 0);
  ...
  if (test('post dispatcher writes both bash audit and cost logs in one pass', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-bash-dispatcher-'));
