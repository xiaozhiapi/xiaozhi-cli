...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-observe-subdir-test-'));
}
...
function gitInit(dir) {
  ...
  const remoteResult = spawnSync(
    'git',
    ['remote', 'add', 'origin', 'https://github.com/example/xiaozhi-test.git'],
...
function readSingleProjectMetadata(homeDir) {
  const projectsDir = path.join(homeDir, '.local', 'share', 'xiaozhi-homunculus', 'projects');
