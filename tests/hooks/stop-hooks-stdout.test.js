 ...
 *
 * Stop hooks follow the XIAOZHI pass-through convention (echo stdin on stdout).
...
const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-stop-stdout-')); // non-git cwd
const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-stop-data-'));
...
function hookEnv() {
  const env = {
    ...
    XIAOZHI_AGENT_DATA_HOME: dataHome,
    CLAUDE_SESSION_ID: `stop-stdout-test-${process.pid}`
  };
  delete env.XIAOZHI_GATEGUARD;
  delete env.XIAOZHI_DISABLED_HOOKS;
  return env;
}
