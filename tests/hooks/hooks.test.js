...
function assertNoProjectDetectionSideEffects(homeDir, testName) {
  const homunculusDir = path.join(homeDir, '.local', 'share', 'xiaozhi-homunculus');
...
async function runTests() {
  ...
  if (
    await asyncTest('exits 0 even with isolated empty HOME', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-iso-start-${Date.now()}`);
  ...
  if (
    await asyncTest('skips template session content', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-tpl-start-${Date.now()}`);
  ...
  if (
    await asyncTest('injects real session content', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-real-start-${Date.now()}`);
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('caps very large session-start context by default', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-large-start-${Date.now()}`);
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('honors XIAOZHI_SESSION_START_MAX_CHARS for injected context', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-max-start-${Date.now()}`);
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          XIAOZHI_SESSION_START_MAX_CHARS: '700'
        });
        assert.strictEqual(result.code, 0);
        const additionalContext = getSessionStartAdditionalContext(result.stdout);
  ...
  if (
    await asyncTest('disables session-start additional context when requested', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-disabled-start-${Date.now()}`);
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          XIAOZHI_SESSION_START_CONTEXT: 'off'
        });
        assert.strictEqual(result.code, 0);
        const additionalContext = getSessionStartAdditionalContext(result.stdout);
  ...
  if (
    await asyncTest('prefers canonical session-data content over legacy duplicates', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-canonical-start-${Date.now()}`);
  ...
  if (
    await asyncTest('strips ANSI escape codes from injected session content', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-ansi-start-${Date.now()}`);
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('skips prior session summary on Desktop SessionStart resume', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-resume-start-${Date.now()}`);
  ...
  if (
    await asyncTest('skips prior session summary on CLI SessionStart resume', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-cli-resume-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('skips prior session summary on clear SessionStart payloads', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-clear-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not log malformed SessionStart stdin content while skipping prior summary', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-invalid-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not fall back to unrelated recent session content', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-cross-project-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject same-project sessions from a different explicit worktree', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-cross-worktree-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('allows project fallback only for legacy sessions without worktree metadata', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-project-only-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('reports learned skills count', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-skills-start-${Date.now()}`);
  ...
  if (
    await asyncTest('injects learned skills into session-start additional context', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-skills-context-${Date.now()}`);
      const learnedDir = path.join(isoHome, '.claude', 'skills', 'learned');
      fs.mkdirSync(learnedDir, { recursive: true });
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
  ...
  if (
    await asyncTest('creates or updates session file', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-session-create-${Date.now()}`);
  ...
  if (
    await asyncTest('includes session ID in filename', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-session-id-${Date.now()}`);
  ...
  if (
    await asyncTest('derives shortId from transcript_path UUID when available', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-session-transcript-${Date.now()}`);
  ...
  if (
    await asyncTest('normalizes transcript UUID shortId to lowercase', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-session-transcript-upper-${Date.now()}`);
      const transcriptUuid = 'ABCDEF12-3456-4789-A012-BCDEF3456789';
  ...
  if (
    await asyncTest('matches getSessionIdShort when transcript UUID equals CLAUDE_SESSION_ID', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-session-transcript-match-${Date.now()}`);
  ...
  if (
    await asyncTest('writes project, branch, and worktree metadata into new session files', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-session-metadata-${Date.now()}`);
  ...
  if (
    await asyncTest('annotates active session file with compaction marker', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-annotate-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('compaction log contains timestamp', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-ts-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      // See: https://github.com/xiaozhiapi/everything-claude-code/issues/103
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('XIAOZHI_OBSERVER_MAX_TURNS'), 'observer-loop should allow max-turn overrides');
      ...
      assert.ok(observerLoopSource.includes('XIAOZHI_SKIP_OBSERVE=1'), 'observer-loop should suppress observe.sh for automated sessions');
      assert.ok(observerLoopSource.includes('XIAOZHI_HOOK_PROFILE=minimal'), 'observer-loop should run automated analysis with the minimal hook profile');
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('detect-project writes project metadata to the registry and project directory', async () => {
      ...
      try {
        ...
        spawnSync('git', ['remote', 'add', 'origin', 'https://github.com/example/xiaozhi-test.git'], { cwd: repoDir, stdio: 'ignore' });
        ...
        const registryPath = path.join(homeDir, '.local', 'share', 'xiaozhi-homunculus', 'projects.json');
        ...
        assert.strictEqual(metadata.remote, 'https://github.com/example/xiaozhi-test.git', 'project.json should include the sanitized remote');
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips minimal hook profile (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips minimal hook profile before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', XIAOZHI_HOOK_PROFILE: 'minimal' }
      });
    })
  )
    passed++;
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips cooperative skip env (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips cooperative skip env before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', XIAOZHI_SKIP_OBSERVE: '1' }
      });
    })
  )
    passed++;
    ...
    await asyncTest('observe.sh skips configured observer-session paths before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: {
          ...
          XIAOZHI_OBSERVE_SKIP_PATHS: ' observer-sessions , .claude-mem '
        },
  ...
  if (
    await asyncTest('only annotates *-session.tmp files, not other .tmp files', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-glob-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('handles no active session files gracefully', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-nosession-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 with empty sessions directory (no recent sessions)', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-empty-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
  ...
  if (
    await asyncTest('does not inject blank template session into context', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-blank-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('annotates only the newest session file when multiple exist', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-multi-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('does not inject empty session file content into context', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-empty-file-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('summary omits Files Modified and Tools Used when none found', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-notools-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('reports available session aliases on startup', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-alias-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('parallel compaction runs all append to log without loss', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-par-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when sessions path is a file (not a directory)', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-blocked-${Date.now()}`);
  ...
  if (
    await asyncTest('excludes session files older than 7 days', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-7day-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('prunes session files older than the retention window', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-prune-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          XIAOZHI_SESSION_RETENTION_DAYS: '30',
        });
  ...
  if (
    await asyncTest('disables pruning when XIAOZHI_SESSION_RETENTION_DAYS=0', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-prune-off-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          XIAOZHI_SESSION_RETENTION_DAYS: '0',
        });

        assert.strictEqual(result.code, 0);
        assert.ok(fs.existsSync(expiredFile), 'Should keep all sessions when retention is opt-out=0');
        assert.ok(result.stderr.includes('Pruning disabled via XIAOZHI_SESSION_RETENTION_DAYS'),
  ...
  if (
    await asyncTest('disables pruning when XIAOZHI_SESSION_RETENTION_DAYS=off', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-prune-offstr-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          XIAOZHI_SESSION_RETENTION_DAYS: 'off',
        });
  ...
  if (
    await asyncTest('falls back to default retention when XIAOZHI_SESSION_RETENTION_DAYS is garbage', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-prune-garbage-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          XIAOZHI_SESSION_RETENTION_DAYS: 'bogus-value',
        });
  ...
  if (
    await asyncTest('injects newest session when multiple recent sessions exist', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-multi-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `xiaozhi-start-unreadable-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when session file is read-only (appendFile fails)', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `xiaozhi-compact-ro-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs warning when existing session file lacks Last Updated field', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-end-nots-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts user messages from role-only format (no type field)', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-role-only-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs "Transcript not found" for nonexistent transcript_path', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-notfound-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts tool name and file path from entry.name/entry.input (not tool_name/tool_input)', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-r70-entryname-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-r71-ss-default-${Date.now()}`);
  ...
  if (
    await asyncTest('extracts user messages from entries where only message.role is user (not type or role)', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-msgrole-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('skips user messages with numeric content (non-string non-array branch)', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `xiaozhi-r81-numcontent-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      const transcriptPath = path.join(isoHome, 'transcript.jsonl');
  ...
  if (
    await asyncTest('collects tool name from entry with tool_name but non-tool_use type', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-r82-toolname-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('preserves file when marker present but regex does not match corrupted template', async () => {
      const isoHome = path.join(os.tmpdir(), `xiaozhi-r82-tmpl-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
