...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-observer-test-'));
}
...
test('observe.sh default throttle is 20 observations per signal', () => {
  const content = fs.readFileSync(observeShPath, 'utf8');
  assert.ok(content.includes('XIAOZHI_OBSERVER_SIGNAL_EVERY_N:-20'), 'Default signal frequency should be every 20 observations');
});
...
test('default cooldown is 60 seconds', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('XIAOZHI_OBSERVER_ANALYSIS_COOLDOWN:-60'), 'Default cooldown should be 60 seconds');
});
...
test('observer-loop.sh defines idle timeout fallback', () => {
  ...
  assert.ok(content.includes('XIAOZHI_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800'), 'Default idle timeout should be 30 minutes');
});
...
test('default max analysis lines is 500', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('XIAOZHI_OBSERVER_MAX_ANALYSIS_LINES:-500'), 'Default should sample last 500 lines');
});

test('analysis temp file is created and cleaned up', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('xiaozhi-observer-analysis'), 'Should create a temp analysis file');
...
test('observer-loop uses project-local temp directory for analysis artifacts', () => {
  ...
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/xiaozhi-observer-analysis.'), 'Analysis temp file should use the project temp dir');
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/xiaozhi-observer-prompt.'), 'Prompt temp file should use the project temp dir');
});
...
test('observe.sh creates counter file and increments on each call', () => {
  ...
  // We use XIAOZHI_SKIP_OBSERVE=0 and minimal JSON so observe.sh processes but
  ...
  fs.writeFileSync(
    ...
    [
      '#!/bin/bash',
      '_xiaozhi_resolve_homunculus_dir() { printf "%s\\n" "$HOME/.local/share/xiaozhi-homunculus"; }',
      ''
    ].join('\n')
  );
  ...
  for (let i = 0; i < 2; i++) {
    spawnSync('bash', [testObserve, 'post'], {
      ...
      env: {
        ...
        XIAOZHI_HOOK_PROFILE: 'standard',
        XIAOZHI_SKIP_OBSERVE: '0',
...
test('claude invocation still includes XIAOZHI_SKIP_OBSERVE and XIAOZHI_HOOK_PROFILE guards', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  ...
  assert.ok(fullCommand.includes('XIAOZHI_SKIP_OBSERVE=1'), 'claude invocation should include XIAOZHI_SKIP_OBSERVE=1 guard');
  assert.ok(fullCommand.includes('XIAOZHI_HOOK_PROFILE=minimal'), 'claude invocation should include XIAOZHI_HOOK_PROFILE=minimal guard');
});
