...
console.log('\n=== Testing XIAOZHI platform value loop docs ===\n');

test('platform value loop doc defines the three-layer XIAOZHI 2.0 direction', () => {
  const source = read('docs/architecture/platform-value-loop.md');

  for (const marker of [
    'Meta-harness',
    'Dedicated XIAOZHI agent',
    'Control pane / agentic IDE',
    'reproducible demo',
    'XIAOZHI can be used full-stack as a meta-harness + agent + control pane',
  ]) {
...
test('Ito example preserves non-advisory and gated-access boundaries', () => {
  ...
  for (const marker of [
    ...
    'do not merge XIAOZHI Tools billing with Ito billing',
  ]) {
...
test('platform value loop does not overclaim release status or trading ability', () => {
  ...
  const forbidden = [
    ...
    'public XIAOZHI skills place trades',
  ];
