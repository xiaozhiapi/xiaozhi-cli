...
for (const relativePath of publicInstallDocs) {
  ...
  test(`${relativePath} documents the short marketplace plugin identifier`, () => {
    assert.ok(content.includes('xiaozhi@xiaozhi'));
  });
}
...
for (const relativePath of manualClaudeSkillInstallDocs) {
  ...
  test(`${relativePath} keeps manual Claude skill installs top-level`, () => {
    assert.ok(
      !/^\s*#?\s*(mkdir\s+-p|md\s+.*|cp\s+.*|copy\s+.*|cpi\s+.*|New-Item\s+.*|Copy-Item\s+.*)\s+.*(~|\$HOME)[\\/]\.claude[\\/]skills[\\/]xiaozhi([\\/]|\b)/mi.test(content),
      'Claude Code does not discover skills installed by commands targeting ~/.claude/skills/xiaozhi'
    );
    assert.ok(
