...
test('token optimization guide separates Claude MCP disables from XIAOZHI config filters', () => {
  ...
  assert.ok(
    source.includes('`XIAOZHI_DISABLED_MCPS` only affects XIAOZHI-generated MCP config output'),
    'Token guide should scope XIAOZHI_DISABLED_MCPS to config generation'
  );
  assert.ok(
...
test('README MCP guidance avoids settings.json disable instructions', () => {
  ...
  assert.ok(
    source.includes('`XIAOZHI_DISABLED_MCPS` is an XIAOZHI install/sync filter, not a live Claude Code toggle.'),
    'README should explain XIAOZHI_DISABLED_MCPS scope'
  );
  assert.ok(
