...
console.log('\n=== Testing XIAOZHI 2.0 release surface ===\n');
...
test('business launch copy stays aligned with the rc.1 public surface', () => {
  ...
  assert.ok(source.includes('XIAOZHI v2.0.0-rc.1'), 'business launch copy should use the rc.1 release');
  ...
  assert.ok(source.includes('https://github.com/xiaozhiapi/xiaozhi-cli'), 'business launch copy should include the public repo URL');
  assert.ok(source.includes('https://github.com/xiaozhiapi/xiaozhi-cli/blob/main/docs/releases/2.0.0-rc.1/release-notes.md'), 'business launch copy should link to the rc.1 release notes');
...
test('announcement drafts avoid live-release claims before publication', () => {
  ...
  for (const relativePath of announcementFiles) {
    ...
    assert.ok(!/XIAOZHI v2\.0\.0-rc\.1 is live\./.test(source), `${relativePath} must not claim rc.1 is live before the release gate completes`);
  }
});
...
test('Hermes setup uses release-candidate wording for the rc.1 surface', () => {
  ...
  assert.ok(source.includes('XIAOZHI v2.0.0-rc.1 documents the Hermes surface'));
...
test('Hermes setup cross-links adjacent migration and architecture docs', () => {
  ...
  assert.ok(!source.includes('0.5. Generate and review artifacts with `xiaozhi migrate plan` /'));
});

test('release docs preserve the XIAOZHI/Hermes boundary', () => {
  const releaseNotes = read('docs/releases/2.0.0-rc.1/release-notes.md');
  assert.ok(releaseNotes.includes('XIAOZHI is the reusable substrate'));
...
test('preview pack manifest assembles release, Hermes, and publication gates', () => {
  ...
  for (const blocker of [
    ...
    'npm `xiaozhi-cli@2.0.0-rc.1`',
    'Claude plugin tag',
    'Codex repo-marketplace distribution evidence',
    'XIAOZHI Tools billing/product readiness'
  ]) {
...
test('owner approval packet consolidates the final gated decisions', () => {
  ...
  const hypergrowth = read('docs/releases/2.0.0/xiaozhi-2-hypergrowth-release-command-center.md');
  ...
  for (const urlSurface of ['GitHub prerelease URL', 'npm rc package URL', 'Claude plugin tag URL', 'Primary launch video URL', 'XIAOZHI Tools billing/readiness URL']) {
...
test('GA roadmap mirrors the current May 19 release evidence', () => {
  const roadmap = read('docs/XIAOZHI-2.0-GA-ROADMAP.md');
...
test('rc.1 quickstart gives a clone-to-cross-harness path', () => {
  ...
  assert.ok(quickstart.includes('git clone https://github.com/xiaozhiapi/xiaozhi-cli.git'));
  assert.ok(quickstart.includes('cd XIAOZHI'));
...
test('release video suite manifest gates the content launch lane', () => {
  ...
  for (const marker of [
    ...
    'XIAOZHI_VIDEO_SOURCE_ROOT',
    'XIAOZHI_VIDEO_RELEASE_SUITE_ROOT',
    'video-use compatible workflow',
    'Self-Eval Gate',
    'Do Not Publish If',
    'renders/xiaozhi-2-primary-launch-rough-v1.mp4',
...
test('release approval gate blocks publication until owner decisions and URLs are final', () => {
  ...
  for (const marker of [
    'xiaozhi.release-approval-gate.v1',
...
test('release video suite public docs do not expose private media paths', () => {
  const releaseVideoDocs = ['docs/releases/2.0.0-rc.1/video-suite-production.md', 'docs/releases/2.0.0/xiaozhi-2-hypergrowth-release-command-center.md'];
...
test('publication readiness checklist gates public release actions on evidence', () => {
  ...
  for (const surface of ['GitHub release', 'npm package', 'Claude plugin', 'Codex plugin', 'Codex repo marketplace', 'OpenCode package', 'XIAOZHI Tools billing reference', 'Announcement copy']) {
  ...
  assert.ok(may15Evidence.includes('XIAOZHI Tools PR #73'));
  assert.ok(may15Evidence.includes('XIAOZHI-Tools PR #75'));
  ...
  assert.ok(source.includes('XIAOZHI-Tools #92 main CI'));
  assert.ok(source.includes('XIAOZHI-Tools #93 main CI'));
...
test('release name and plugin publication checklist freezes rc.1 surfaces', () => {
  ...
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  const referenceArchitecture = read('docs/XIAOZHI-2.0-REFERENCE-ARCHITECTURE.md');

  for (const value of [
    'Ship `v2.0.0-rc.1` as **XIAOZHI**',
    '`xiaozhiapi/XIAOZHI`',
    '`xiaozhi-cli`',
    '`xiaozhi` on npm is occupied',
    '`@xiaozhiapi/xiaozhi` is unclaimed on npm',
    'Claude plugin',
    'Codex plugin',
...
test('active release identity surfaces use canonical XIAOZHI repo URLs', () => {
  const activeFiles = [
    'README.md',
  ...
  for (const relativePath of activeFiles) {
    const source = read(relativePath);
    if (source.includes('xiaozhiapi/everything-claude-code')) {
      offenders.push(relativePath);
    }
  }
