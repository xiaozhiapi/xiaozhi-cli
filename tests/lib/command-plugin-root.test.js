...
const { INLINE_RESOLVE } = require('../../scripts/lib/resolve-xiaozhi-root');
...
test('sessions command uses shared inline resolver in all node scripts', () => {
  ...
  assert.strictEqual((sessionsDoc.match(/\['marketplaces','xiaozhi'\]/g) || []).length, 6);
  assert.strictEqual((sessionsDoc.match(/\['marketplaces','everything-claude-code'\]/g) || []).length, 6);
  assert.strictEqual((sessionsDoc.match(/\['xiaozhi','everything-claude-code'\]/g) || []).length, 6);
});
...
test('skill-health command uses shared inline resolver in all shell snippets', () => {
  ...
  assert.strictEqual((skillHealthDoc.match(/\['marketplaces','xiaozhi'\]/g) || []).length, 3);
  assert.strictEqual((skillHealthDoc.match(/\['marketplaces','everything-claude-code'\]/g) || []).length, 3);
  assert.strictEqual((skillHealthDoc.match(/\['xiaozhi','everything-claude-code'\]/g) || []).length, 3);
});
...
test('instinct-status command uses shared inline resolver (no stale legacy fallback) (#2037)', () => {
  ...
  assert.strictEqual((instinctStatusDoc.match(/\['marketplaces','xiaozhi'\]/g) || []).length, 1);
  assert.strictEqual((instinctStatusDoc.match(/\['marketplaces','everything-claude-code'\]/g) || []).length, 1);
  assert.strictEqual((instinctStatusDoc.match(/\['xiaozhi','everything-claude-code'\]/g) || []).length, 1);
...
test('inline resolver covers current and legacy marketplace plugin roots', () => {
  assert.ok(INLINE_RESOLVE.includes("'marketplaces','xiaozhi'"));
