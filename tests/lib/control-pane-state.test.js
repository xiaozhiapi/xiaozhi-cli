...
async function writeSampleXiaozhiDatabase(dbPath) {
  ...
  insertSession.run([
    'lead-hermes',
    'Coordinate Hermes desktop and XIAOZHI release work',
    'XIAOZHI',
    '2.0-control-pane',
    'claude',
    'claude',
    ...
    'running',
    4242,
    '/tmp/xiaozhi-worktrees/hermes',
    'xiaozhi/hermes-control-pane',
    'main',
    1200,
    800,
    2000,
  ...
  insertEntity.run([
    ...
    '/notes/hermes.md',
    'How Affaan routes Hermes Desktop, Zellij panes, Devin-style delegation, and XIAOZHI release control work.',
  ...
  db.run(
    ...
    [
      ...
      3,
      1,
      'Hermes Desktop and XIAOZHI should share recall before dispatching work.',
...
async function writeSampleWorkItemsDatabase(dbPath) {
  ...
  insertWorkItem.run([
    ...
    'github-pr',
    '2131',
    'Merge XIAOZHI control pane',
    'done',
    'normal',
    'https://github.com/xiaozhiapi/xiaozhi-cli/pull/2131',
    'affaan',
    '/repo/xiaozhi',
    null,
    JSON.stringify({
...
async function runTests() {
  ...
  if (await test('shows configured connectors even when the SQLite database is missing', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-control-pane-missing-db-'));
    ...
    const observations = [
      {
        ...
        priority: 1,
        pinned: false,
        summary: 'Ito should expose market backtesting through XIAOZHI tools.',
      },
    ];
