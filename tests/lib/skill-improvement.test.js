...
test('observation layer writes and reads structured skill outcomes', () => {
  const projectRoot = makeProjectRoot('xiaozhi-skill-observe-');
  ...
  try {
    ...
    assert.strictEqual(records[0].schemaVersion, 'xiaozhi.skill-observation.v1');
    ...
    assert.strictEqual(getSkillObservationsPath({ projectRoot }), path.join(projectRoot, '.claude', 'xiaozhi', 'skills', 'observations.jsonl'));
  } finally {
    cleanup(projectRoot);
  }
});
...
test('health inspector traces recurring failures for a skill across runs', () => {
  const projectRoot = makeProjectRoot('xiaozhi-skill-health-');
...
test('amendify proposes SKILL.md patch content from failure evidence', () => {
  ...
  assert.strictEqual(proposal.schemaVersion, 'xiaozhi.skill-amendment-proposal.v1');
...
test('evaluation scaffold compares amended and baseline performance', () => {
  ...
  assert.strictEqual(evaluation.schemaVersion, 'xiaozhi.skill-evaluation.v1');
