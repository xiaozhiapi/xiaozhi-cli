...
function runTests() {
  ...
  if (test('resolves cursor adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.cursor', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves claude adapter root and install-state path from home dir', () => {
    ...
    const root = adapter.resolveRoot({ homeDir, repoRoot: '/repo/xiaozhi' });
    const statePath = adapter.getInstallStatePath({ homeDir, repoRoot: '/repo/xiaozhi' });

    assert.strictEqual(root, path.join(homeDir, '.claude'));
    assert.strictEqual(statePath, path.join(homeDir, '.claude', 'xiaozhi', 'install-state.json'));
  })) passed++; else failed++;

  if (test('plans claude rules and skills under XIAOZHI-managed subdirectories', () => {
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'xiaozhi')
      )),
      'Should install bundled Claude rules under rules/xiaozhi'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
        && operation.destinationPath === path.join(homeDir, '.claude', 'skills', 'xiaozhi', 'tdd-workflow')
      )),
      'Should install bundled Claude skills under skills/xiaozhi'
    );
  })) passed++; else failed++;
  ...
  if (test('plans scaffold operations and flattens native target roots', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.cursor', 'xiaozhi-install-state.json'));
  ...
  if (test('does not install root AGENTS.md into Cursor nested context', () => {
    ...
    assert.ok(
      ...
      'Cursor installs should not copy XIAOZHI root AGENTS.md into host project context'
    );
    assert.ok(
      !plan.operations.some(operation => (
  ...
  if (test('plans cursor agents with xiaozhi-prefixed filenames to avoid agent collisions', () => {
    const repoRoot = path.join(__dirname, '..', '..');
    const projectRoot = '/workspace/app';
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(projectRoot, '.cursor', 'agents', 'xiaozhi-architect.md')
      )),
      'Should prefix Cursor agent files with xiaozhi-'
    );
    assert.ok(
      !plan.operations.some(operation => (
  ...
  if (test('exposes validate and planOperations on adapters', () => {
    ...
    assert.deepStrictEqual(
      claudeAdapter.validate({ homeDir: '/Users/example', repoRoot: '/repo/xiaozhi' }),
      []
    );
    ...
    assert.deepStrictEqual(
      cursorAdapter.validate({ projectRoot: '/workspace/app', repoRoot: '/repo/xiaozhi' }),
      []
    );
  })) passed++; else failed++;
  ...
  if (test('resolves codebuddy adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.codebuddy', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves gemini adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.gemini', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves joycode adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.joycode', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves qwen adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(statePath, path.join(homeDir, '.qwen', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves zed adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.zed', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans codebuddy rules with flat namespaced filenames', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.codebuddy', 'xiaozhi-install-state.json'));
  ...
  if (test('plans joycode commands, agents, skills, and flattened rules', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.joycode', 'xiaozhi-install-state.json'));
  ...
  if (test('plans qwen commands, agents, skills, and native config under home root', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(homeDir, '.qwen', 'xiaozhi-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
  ...
  if (test('plans zed project settings, commands, agents, skills, and flattened rules', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.zed', 'xiaozhi-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
  ...
  if (test('exposes validate and planOperations on codebuddy adapter', () => {
    ...
    assert.deepStrictEqual(
      codebuddyAdapter.validate({ projectRoot: '/workspace/app', repoRoot: '/repo/xiaozhi' }),
      []
    );
  })) passed++; else failed++;

  if (test('every schema target enum value has a matching adapter (regression guard)', () => {
    const schemaPath = path.join(__dirname, '..', '..', 'schemas', 'xiaozhi-install-config.schema.json');
  ...
  if (test('resolves claude-project adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.claude', 'xiaozhi', 'install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans claude-project rules and skills under project-scope XIAOZHI-managed subdirectories', () => {
    const repoRoot = path.join(__dirname, '..', '..');
    const projectRoot = '/workspace/app';
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.claude', 'xiaozhi', 'install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
        && operation.destinationPath === path.join(projectRoot, '.claude', 'rules', 'xiaozhi')
      )),
      'Should install bundled rules under project-scope rules/xiaozhi'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
        && operation.destinationPath === path.join(projectRoot, '.claude', 'skills', 'xiaozhi', 'tdd-workflow')
      )),
      'Should install bundled skills under project-scope skills/xiaozhi'
    );
  })) passed++; else failed++;
  ...
  if (test('resolves opencode adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(statePath, path.join(homeDir, '.opencode', 'xiaozhi-install-state.json'));
  })) passed++; else failed++;
