...
function runTests() {
  ...
  if (
    test('returns path containing xiaozhi-metrics-', () => {
      const p = getBridgePath('test-session');
      assert.ok(p.includes('xiaozhi-metrics-'), `Expected xiaozhi-metrics- in path, got: ${p}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveSessionId uses XIAOZHI_SESSION_ID env var', () => {
      const original = process.env.XIAOZHI_SESSION_ID;
      try {
        process.env.XIAOZHI_SESSION_ID = 'env-session-42';
        const result = resolveSessionId();
        assert.strictEqual(result, 'env-session-42');
      } finally {
        if (original === undefined) {
          delete process.env.XIAOZHI_SESSION_ID;
        } else {
          process.env.XIAOZHI_SESSION_ID = original;
        }
      }
    })
  )
