/**
 * Tests for scripts/lib/resolve-xiaozhi-root.js
 *
 * Covers the XIAOZHI root resolution fallback chain:
...
const { resolveEccRoot, INLINE_RESOLVE } = require('../../scripts/lib/resolve-xiaozhi-root');
...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-root-test-'));
}
...
function runTests() {
  console.log('\n=== Testing resolve-xiaozhi-root.js ===\n');
  ...
  if (test('finds current plugin install at ~/.claude/plugins/xiaozhi', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['xiaozhi']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('finds current plugin install at ~/.claude/plugins/xiaozhi@xiaozhi', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['xiaozhi@xiaozhi']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds marketplace current plugin install at ~/.claude/plugins/marketplaces/xiaozhi', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'xiaozhi']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('prefers exact legacy plugin install over plugin cache', () => {
    ...
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'xiaozhi']);
      setupPluginCache(homeDir, 'xiaozhi', 'xiaozhiapi', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('discovers plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'xiaozhi', 'xiaozhiapi', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
