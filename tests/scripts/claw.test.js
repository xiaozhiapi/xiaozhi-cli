...
// Test helper — matches XIAOZHI's custom test pattern
...
function runTests() {
  ...
  if (test('loadXiaozhiContext() returns "" when no skills specified', () => {
    const result = loadXiaozhiContext('');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadXiaozhiContext() skips missing skill directories gracefully', () => {
    const result = loadXiaozhiContext('nonexistent-skill-xyz');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadXiaozhiContext() concatenates multiple skill files', () => {
    // Use real skills from the XIAOZHI repo if they exist
    ...
    const result = loadXiaozhiContext(twoSkills);
  ...
  if (test('module exports all required functions', () => {
    ...
    const required = [
      ...
      'appendTurn', 'loadXiaozhiContext', 'askClaude', 'main'
    ];
