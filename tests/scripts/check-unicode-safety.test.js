...
function runCheck(root, args = []) {
  return spawnSync('node', [scriptPath, ...args], {
    env: {
      ...process.env,
      XIAOZHI_UNICODE_SCAN_ROOT: root,
    },
    encoding: 'utf8',
  });
}
...
if (
  test('fails on invisible unicode and emoji before cleanup', () => {
    const root = makeTempRoot('xiaozhi-unicode-check-');
...
if (
  test('write mode removes emoji and invisible unicode', () => {
    const root = makeTempRoot('xiaozhi-unicode-fix-');
    fs.mkdirSync(path.join(root, 'docs'), { recursive: true });
    fs.writeFileSync(path.join(root, 'docs', 'guide.md'), `> ${warningEmoji} Important launch note\n`);
...
if (
  test('write mode does not rewrite executable files', () => {
    const root = makeTempRoot('xiaozhi-unicode-code-');
    fs.mkdirSync(path.join(root, 'scripts'), { recursive: true });
...
if (
  test('plain symbols like copyright remain allowed', () => {
    const root = makeTempRoot('xiaozhi-unicode-symbols-');
    fs.mkdirSync(path.join(root, 'docs'), { recursive: true });
    fs.writeFileSync(path.join(root, 'docs', 'legal.md'), 'Copyright © XIAOZHI\nTrademark ® XIAOZHI\n');
...
for (const { codePoint, label } of NEWLY_COVERED_RANGES) {
  if (
    test(`detects ${label}`, () => {
      const root = makeTempRoot('xiaozhi-unicode-newly-covered-');
      fs.mkdirSync(path.join(root, 'docs'), { recursive: true });
...
if (
  test('write mode strips newly-covered invisibles from markdown', () => {
    const root = makeTempRoot('xiaozhi-unicode-newly-covered-write-');
    fs.mkdirSync(path.join(root, 'docs'), { recursive: true });
...
if (
  test('skips Python virtual environments', () => {
    const root = makeTempRoot('xiaozhi-unicode-venv-');
