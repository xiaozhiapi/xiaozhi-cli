/**
 * Source-level tests for scripts/sync-xiaozhi-to-codex.sh
 */
...
const scriptPath = path.join(__dirname, '..', '..', 'scripts', 'sync-xiaozhi-to-codex.sh');
...
function runTests() {
  console.log('\n=== Testing sync-xiaozhi-to-codex.sh ===\n');
  ...
  if (test('sync script avoids GNU-only grep -P parsing', () => {
    assert.ok(!source.includes('grep -oP'), 'sync-xiaozhi-to-codex.sh should remain portable across BSD and GNU environments');
  })) passed++; else failed++;
