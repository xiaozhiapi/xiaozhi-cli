...
function runTests() {
  ...
  if (test('README documents reset and uninstall flow', () => {
    assert.ok(
      readme.includes('### Reset / Uninstall XIAOZHI'),
    ...
    assert.ok(
      readme.includes('node scripts/xiaozhi.js list-installed'),
      'README should document install-state inspection before reinstalling'
    );
    assert.ok(
      readme.includes('node scripts/xiaozhi.js doctor'),
      'README should document doctor before reinstalling'
    );
    assert.ok(
      readme.includes('XIAOZHI only removes files recorded in its install-state.'),
  ...
  if (test('README documents low-context no-hooks install path', () => {
    ...
    assert.ok(
      readme.includes('npx xiaozhi-install --profile minimal --target claude'),
  ...
  if (test('README documents consult-based component discovery', () => {
    ...
    assert.ok(
      readme.includes('npx xiaozhi consult "security reviews" --target claude'),
  ...
  if (test('README documents Cursor agent namespace and loading caveat', () => {
    assert.ok(
      readme.includes('`.cursor/agents/xiaozhi-*.md`'),
    ...
    assert.ok(
      readme.includes('XIAOZHI does not install root `AGENTS.md` into `.cursor/`.'),
  ...
  if (test('README explains plugin-path cleanup and rules scoping', () => {
    ...
    assert.ok(
      readme.includes('~/.claude/rules/xiaozhi/'),
      'README should steer plugin-path rules into an XIAOZHI-owned namespace'
    );
  })) passed++; else failed++;

  if (test('rules README mirrors XIAOZHI namespaced install path', () => {
    assert.ok(
      rulesReadme.includes('mkdir -p ~/.claude/rules/xiaozhi'),
      'rules README should create the XIAOZHI-owned user-level rules namespace'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/common ~/.claude/rules/xiaozhi/'),
      'rules README should copy common rules under ~/.claude/rules/xiaozhi/'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/typescript ~/.claude/rules/xiaozhi/'),
      'rules README should copy language rules under ~/.claude/rules/xiaozhi/'
    );
    assert.ok(
      rulesReadme.includes('mkdir -p .claude/rules/xiaozhi'),
      'rules README should document the project-local XIAOZHI namespace'
    );
    assert.ok(
