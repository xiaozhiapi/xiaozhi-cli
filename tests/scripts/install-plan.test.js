...
function runTests() {
  ...
  if (test('shows help with no arguments', () => {
    ...
    assert.ok(result.stdout.includes('Inspect XIAOZHI selective-install manifests'));
  })) passed++; else failed++;
  ...
  if (test('loads planning intent from xiaozhi-install.json', () => {
    const configDir = path.join(__dirname, '..', 'fixtures', 'tmp-install-plan-config');
    const configPath = path.join(configDir, 'xiaozhi-install.json');
  ...
  if (test('auto-detects planning intent from project xiaozhi-install.json', () => {
