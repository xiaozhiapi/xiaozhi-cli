...
function makeRecord({ repoRoot, homeDir, projectRoot, adapter, request, resolution, operations }) {
  ...
  const installStatePath = adapter.kind === 'project'
    ? path.join(targetRoot, 'xiaozhi-install-state.json')
    : path.join(targetRoot, 'xiaozhi', 'install-state.json');
...
function runTests() {
  ...
  if (test('parseArgs reads repo-root, target, dry-run, and json flags', () => {
    ...
    assert.strictEqual(parsed.repoRoot, '/tmp/xiaozhi');
  ...
  if (test('deriveRepoRootFromState uses sourcePath and sourceRelativePath', () => {
    const state = {
      operations: [
        {
          sourcePath: path.join('/tmp', 'xiaozhi', 'scripts', 'setup-package-manager.js'),
    ...
    assert.strictEqual(
      ...
      path.resolve(path.join('/tmp', 'xiaozhi'))
    );
  })) passed += 1; else failed += 1;
  ...
  if (test('deriveRepoRootFromState fails when source metadata is unavailable', () => {
    assert.throws(
      ...
      /Unable to infer XIAOZHI repo root/
    );
  })) passed += 1; else failed += 1;
  ...
  if (test('determineInstallCwd uses the project root for project installs', () => {
    ...
    assert.strictEqual(determineInstallCwd(record, '/tmp/xiaozhi'), path.join('/tmp', 'project'));
  })) passed += 1; else failed += 1;
  ...
  if (test('runAutoUpdate rejects mixed inferred repo roots', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        /Multiple XIAOZHI repo roots detected/
      );
    } finally {
      cleanup(homeDir);
      cleanup(projectRoot);
            ...
            if (command === process.execPath) {
              return {
                stdout: JSON.stringify({
                  dryRun: false,
                  result: {
                    installStatePath: path.join(projectRoot, '.cursor', 'xiaozhi-install-state.json'),
                  },
                }),
                stderr: '',
              };
