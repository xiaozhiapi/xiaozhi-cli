...
function runTests() {
  ...
  if (test('installs Claude rules and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'rules', 'xiaozhi', 'common', 'coding-style.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'rules', 'xiaozhi', 'typescript', 'testing.md')));
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'skills', 'xiaozhi', 'tdd-workflow', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'skills', 'xiaozhi', 'coding-standards', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'plugin.json')));

      const statePath = path.join(homeDir, '.claude', 'xiaozhi', 'install-state.json');
      ...
      assert.ok(
        state.operations.some(operation => (
          operation.destinationPath === path.join(claudeRoot, 'rules', 'xiaozhi', 'common', 'coding-style.md')
        )),
  ...
  if (test('installs Cursor configs and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(projectDir, '.cursor', 'agents', 'xiaozhi-architect.md')));
      ...
      const statePath = path.join(projectDir, '.cursor', 'xiaozhi-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs Antigravity configs and writes install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(projectDir, '.agent', 'xiaozhi-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs JoyCode profile through managed install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(projectDir, '.joycode', 'xiaozhi-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs Qwen profile through managed home install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(homeDir, '.qwen', 'xiaozhi-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('supports dry-run without mutating the target project', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(projectDir, '.cursor', 'hooks.json')));
      assert.ok(!fs.existsSync(path.join(projectDir, '.cursor', 'xiaozhi-install-state.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('supports manifest profile dry-runs through the installer', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'xiaozhi', 'install-state.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('installs manifest profiles and writes non-legacy install-state', () => {
    ...
    try {
      ...
      const state = readJson(path.join(claudeRoot, 'xiaozhi', 'install-state.json'));
  ...
  if (test('installs antigravity manifest profiles while skipping only unsupported modules', () => {
    ...
    try {
      ...
      const state = readJson(path.join(projectDir, '.agent', 'xiaozhi-install-state.json'));
      assert.strictEqual(state.request.profile, 'core');
      assert.strictEqual(state.request.legacyMode, false);
  ...
  if (test('installs explicit modules for cursor using manifest operations', () => {
    ...
    try {
      ...
      const state = readJson(path.join(projectDir, '.cursor', 'xiaozhi-install-state.json'));
      ...
      assert.ok(state.resolution.selectedModules.includes('platform-configs'));
      assert.ok(
        !state.operations.some(operation => operation.destinationPath.endsWith('xiaozhi-install-state.json')),
  ...
  if (test('filters copied mcp config files when XIAOZHI_DISABLED_MCPS is set', () => {
    ...
    const installStatePath = path.join(tempDir, 'installed', 'xiaozhi-install-state.json');
    const previousValue = process.env.XIAOZHI_DISABLED_MCPS;
    ...
    try {
      ...
      process.env.XIAOZHI_DISABLED_MCPS = 'github,memory';

      applyInstallPlan({
        targetRoot: path.join(tempDir, 'installed'),
        installStatePath,
        statePreview: {
          schemaVersion: 'xiaozhi.install.v1',
      ...
      if (previousValue === undefined) {
        delete process.env.XIAOZHI_DISABLED_MCPS;
      } else {
        process.env.XIAOZHI_DISABLED_MCPS = previousValue;
      }
      cleanup(tempDir);
    }
  })) passed++; else failed++;
  ...
  if (test('ignores malformed existing settings.json during claude install', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'xiaozhi', 'install-state.json')), 'install state should still be written');
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('fails when source hooks.json root is not an object before copying files', () => {
    ...
    const installStatePath = path.join(targetRoot, 'xiaozhi', 'install-state.json');
  ...
  if (test('installs from xiaozhi-install.json and persists component selections', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    const configPath = path.join(projectDir, 'xiaozhi-install.json');
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.claude', 'skills', 'xiaozhi', 'security-review', 'SKILL.md')));
      assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'skills', 'xiaozhi', 'dmux-workflows', 'SKILL.md')));

      const state = readJson(path.join(homeDir, '.claude', 'xiaozhi', 'install-state.json'));
  ...
  if (test('auto-detects xiaozhi-install.json from the project root', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    const configPath = path.join(projectDir, 'xiaozhi-install.json');
