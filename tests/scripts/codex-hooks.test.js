...
const syncScript = path.join(repoRoot, 'scripts', 'sync-xiaozhi-to-codex.sh');
...
function makeHermeticCodexEnv(homeDir, codexDir, extraEnv = {}) {
  ...
  return {
    ...
    XIAOZHI_GLOBAL_HOOKS_DIR: hooksDir,
    CLAUDE_PACKAGE_MANAGER: 'npm',
    CLAUDE_CODE_PACKAGE_MANAGER: 'npm',
    LANG: 'C.UTF-8',
    LC_ALL: 'C.UTF-8',
...
function seedPluginCache(codexDir, manifest, files = []) {
  const cacheDir = path.join(codexDir, 'plugins', 'cache', 'xiaozhi', 'xiaozhi', packageVersion);
...
const cacheManifestWithLocalRefs = {
  ...
  mcpServers: './.mcp.json',
  interface: {
    composerIcon: './assets/xiaozhi-icon.svg',
...
if (
  test('check-plugin-cache fails when the installed cache is missing manifest-referenced files', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /sync-xiaozhi-to-codex\.sh/);
    } finally {
      cleanup(homeDir);
    }
  })
...
if (
  test('check-plugin-cache reports a missing installed cache clearly', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /codex plugin marketplace add xiaozhiapi\/XIAOZHI/);
    } finally {
      cleanup(homeDir);
    }
  })
  ...
  test('install-global-git-hooks.sh handles quoted hook paths without shell injection', () => {
    ...
    try {
      const result = runBash(installScript, [], {
        HOME: homeDir,
        XIAOZHI_GLOBAL_HOOKS_DIR: weirdHooksDir,
      });
...
if (
  test('merge-mcp-config no-ops after all recommended servers are present', () => {
    ...
    try {
      ...
      assert.match(second.stdout, /All XIAOZHI MCP servers already present/);
...
if (
  test('merge-mcp-config repairs the invalid exa url entry from earlier XIAOZHI versions (#2224)', () => {
...
if (
  test('merge-mcp-config update dry-run refreshes managed sections and leaves user servers alone', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /\[mcp_servers\.chrome-devtools\]/);
      // Retired servers are no longer XIAOZHI-managed: never removed or re-added.
  ...
  test('merge-mcp-config removes disabled servers without appending replacements', () => {
    ...
    try {
      ...
      const result = runNode(mergeMcpConfigScript, [configPath], {
        ...
        XIAOZHI_DISABLED_MCPS: 'chrome-devtools',
      });

      assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
      assert.match(result.stdout, /Disabled via XIAOZHI_DISABLED_MCPS/);
...
if (
  test('sync installs the missing Codex baseline and accepts the legacy context7 MCP section', () => {
    ...
    try {
      ...
      assert.match(syncedAgents, /^# Everything Claude Code \(XIAOZHI\) — Agent Instructions/m);
      assert.match(syncedAgents, /^# Codex Supplement \(From XIAOZHI \.codex\/AGENTS\.md\)/m);
