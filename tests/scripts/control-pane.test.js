...
async function writeMinimalDatabase(dbPath) {
  ...
  db.run(`
    ...
    INSERT INTO sessions (
      ...
      'session-a', 'Build the control pane', 'codex', 'codex', '["codex"]', '/repo/xiaozhi',
...
function waitForCliReady(child) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', chunk => {
      ...
      if (!settled && stdout.includes('XIAOZHI Control Pane:') && stdout.includes('Actions:')) {
        settled = true;
        clearTimeout(timer);
...
async function runTests() {
  ...
  if (await test('serves HTML and snapshot JSON from a temp XIAOZHI database', async () => {
    ...
    try {
      ...
      try {
        ...
        assert.ok(html.includes('XIAOZHI Control Pane'));
        ...
        assert.strictEqual(snapshot.schemaVersion, 'xiaozhi.control-pane.snapshot.v1');
  ...
  if (await test('serves health, asset, not-found, invalid body, and read-only action responses', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'xiaozhi-control-pane-routes-'));
    ...
    try {
      ...
      try {
        ...
        try {
          const realAsset = await fetchLocal(`${realAssetApp.url}/assets/xiaozhi-icon.svg`);
        ...
        const missingAsset = await fetchLocal(`${app.url}/assets/xiaozhi-icon.svg`);
  ...
  if (await test('runAction captures success, failure, and bounded output', async () => {
    ...
    const spawnError = await runAction({
      id: 'spawn-error',
      command: 'definitely-not-xiaozhi-control-pane-command',
      args: [],
      cwd: repoRoot,
    });
  ...
  if (await test('CLI starts a read-only local server and shuts down on SIGTERM', async () => {
    ...
    try {
      ...
      assert.match(ready.stdout, /XIAOZHI Control Pane: http:\/\/127\.0\.0\.1:\d+/);
