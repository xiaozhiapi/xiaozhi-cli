...
function runTests() {
  ...
  if (test('GitHub Integration category scores against a fully-wired consumer fixture', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'xiaozhi', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'xiaozhi', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'xiaozhi' }, null, 2)
      );
  ...
  if (test('detects repo mode from structural markers when package name differs', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(projectRoot, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'xiaozhi' }, null, 2));
      fs.writeFileSync(
        path.join(projectRoot, 'package.json'),
  ...
  if (test('audits consumer projects from cwd instead of the XIAOZHI repo root', () => {
  ...
  if (test('prints no top actions when consumer checks all pass', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'xiaozhi@xiaozhi'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'xiaozhi@xiaozhi', 'plugin.json'),
        JSON.stringify({ name: 'xiaozhi' }, null, 2)
      );
      fs.mkdirSync(path.join(projectRoot, '.claude'), { recursive: true });
  ...
  if (test('detects Claude plugin installs from installed_plugins.json', () => {
    ...
    const installRoot = path.join(pluginsDir, 'cache', 'everything-claude-code', 'xiaozhi', '2.0.0');

    try {
      fs.mkdirSync(path.join(installRoot, '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(installRoot, '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'xiaozhi', version: '2.0.0' }, null, 2)
      );
      fs.writeFileSync(
        path.join(pluginsDir, 'installed_plugins.json'),
        JSON.stringify({
          plugins: {
            'xiaozhi@everything-claude-code': [
              { installPath: path.join('cache', 'everything-claude-code', 'xiaozhi', '2.0.0') },
            ],
          },
        }, null, 2)
      );
      ...
      try {
        ...
        assert.ok(found);
        assert.ok(found.includes(`${path.sep}cache${path.sep}everything-claude-code${path.sep}xiaozhi${path.sep}2.0.0${path.sep}`));
      } finally {
  ...
  if (test('detects newest Claude plugin install from cache marketplace layout', () => {
    ...
    const pluginRoot = path.join(homeDir, '.claude', 'plugins', 'cache', 'everything-claude-code', 'xiaozhi');
    ...
    try {
      for (const version of ['1.8.0', '1.10.0']) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({ name: 'xiaozhi', version }, null, 2)
        );
      }
