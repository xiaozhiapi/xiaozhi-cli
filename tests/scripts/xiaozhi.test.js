...
const SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'xiaozhi.js');
...
function main() {
  ...
  const tests = [
    ['shows top-level help', () => {
      ...
      assert.match(result.stdout, /XIAOZHI selective-install CLI/);
    ...
    ['delegates lifecycle commands', () => {
      const homeDir = createTempDir('xiaozhi-cli-home-');
      const projectRoot = createTempDir('xiaozhi-cli-project-');
    ...
    ['delegates session-inspect command', () => {
      ...
      assert.strictEqual(payload.workers[0].branch, 'feat/xiaozhi-cli');
    }],
    ['delegates loop-status command', () => {
      const homeDir = createTempDir('xiaozhi-cli-home-');
      const transcriptDir = path.join(homeDir, '.claude', 'projects', '-tmp-xiaozhi');
    ...
    ['delegates security-ioc-scan command', () => {
      const projectRoot = createTempDir('xiaozhi-cli-ioc-scan-');
      fs.writeFileSync(
