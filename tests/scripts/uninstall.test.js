...
function runTests() {
  ...
  if (test('uninstalls files from a real install-apply state and preserves unrelated files', () => {
    ...
    try {
      ...
      const statePath = path.join(normalizedProjectRoot, '.cursor', 'xiaozhi-install-state.json');
  ...
  if (test('reverses non-copy operations and keeps unrelated files', () => {
    ...
    try {
      ...
      const statePath = path.join(normalizedTargetRoot, 'xiaozhi-install-state.json');
