...
function runTests() {
  ...
  if (test('seeded approved release passes every publication approval check', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'xiaozhi.release-approval-gate.v1');
    ...
    try {
      seedRepo(rootDir, {
        [`${RELEASE_DIR}/release-url-ledger-2026-05-19.md`]: [
          ...
          '| --- | --- | --- |',
          '| GitHub prerelease | https://github.com/xiaozhiapi/xiaozhi-cli/releases/tag/v2.0.0-rc.1 | must return the prerelease |',
        ].join('\n'),
      });
