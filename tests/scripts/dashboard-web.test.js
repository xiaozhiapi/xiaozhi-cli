...
test('readFrontmatter parses simple frontmatter', () => {
  ...
  testRoot = createTempDir('xiaozhi-test-');
...
test('renderHTML includes LANG and LANG_KEYS in the output', () => {
  ...
  const html = renderHTML(data);
  assert.ok(html.includes('XIAOZHI Capabilities'));
...
test('renderHTML includes the dashboard title and footer', () => {
  ...
  const html = renderHTML(data);
  assert.ok(html.includes('XIAOZHI Capabilities'));
  assert.ok(html.includes('github.com/xiaozhiapi/xiaozhi-cli'));
});
  ...
  testServer.listen(0, () => {
    ...
    http.get(`http://localhost:${port}/`, (res) => {
      ...
      res.on('end', () => {
        ...
        assert.ok(body.includes('XIAOZHI Capabilities'));
        testServer.close();
        done();
      });
    });
