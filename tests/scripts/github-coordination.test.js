...
const key = args.join(' ');
const logPath = process.env.XIAOZHI_GH_SHIM_LOG;
if (logPath) {
    ...
    await test('claims an epic issue, updates GitHub state, and caches a work item', async () => {
      ...
      try {
        ...
        const issueView = {
          ...
          body: epicBody,
          url: 'https://github.com/xiaozhiapi/xiaozhi-cli/issues/12',
          state: 'OPEN',
          labels: [{ name: 'epic' }],
        ...
        const shim = writeGhShim(rootDir, {
          'issue view 12 --repo xiaozhiapi/XIAOZHI --json number,title,body,url,state,labels,author,updatedAt,assignees': issueView
        });

        const result = run(['claim', '12', '--repo', 'xiaozhiapi/XIAOZHI', '--actor', 'codex', '--db', dbPath, '--json'], {
          cwd: rootDir,
          env: {
            XIAOZHI_GH_SHIM: shim.shimPath,
            XIAOZHI_GH_SHIM_LOG: shim.logPath
          }
        });
    ...
    await test('unblocks an epic when dependencies are closed', async () => {
      ...
      try {
        const blockedBody = [
          ...
          'Dependencies: #2',
          '',
          '<!-- xiaozhi-coordination:start -->',
          '```json',
          JSON.stringify(
            {
              schemaVersion: 'xiaozhi.github.coordination.v1',
              kind: 'epic',
              status: 'blocked',
              owner: 'codex',
          ...
          '```',
          '<!-- xiaozhi-coordination:end -->'
        ].join('\n');
        const openIssue = {
          number: 1,
          title: 'Release readiness',
          body: blockedBody,
          url: 'https://github.com/xiaozhiapi/xiaozhi-cli/issues/1',
          state: 'OPEN',
        ...
        const closedDependency = {
          ...
          url: 'https://github.com/xiaozhiapi/xiaozhi-cli/issues/2',
          state: 'CLOSED',
        ...
        const shim = writeGhShim(rootDir, {
          'issue list --repo xiaozhiapi/XIAOZHI --state all --limit 100 --json number,title,body,url,state,labels,author,updatedAt,assignees': [openIssue, closedDependency],
          'issue view 1 --repo xiaozhiapi/XIAOZHI --json number,title,body,url,state,labels,author,updatedAt,assignees': openIssue
        });

        const result = run(['unblock', '--repo', 'xiaozhiapi/XIAOZHI', '--db', dbPath, '--json'], {
          cwd: rootDir,
          env: {
            XIAOZHI_GH_SHIM: shim.shimPath,
            XIAOZHI_GH_SHIM_LOG: shim.logPath
