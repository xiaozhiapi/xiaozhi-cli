# .codex-plugin — Codex Native Plugin for XIAOZHI

This directory contains the **Codex plugin manifest** for XIAOZHI.
...
codex plugin marketplace add xiaozhiapi/XIAOZHI

# Or add a local checkout while developing
codex plugin marketplace add /absolute/path/to/XIAOZHI
```

The marketplace entry points at `plugins/xiaozhi/` — Codex does not discover
...
[#2128](https://github.com/xiaozhiapi/xiaozhi-cli/issues/2128)). That thin plugin folder
...
`xiaozhi` from the plugin directory.

After install, `codex plugin list` is only a registration check. From an XIAOZHI
...
> `npm install && bash scripts/sync-xiaozhi-to-codex.sh`.
...
repository shape: `plugins/xiaozhi/.codex-plugin/plugin.json`,
`plugins/xiaozhi/skills/`, and the supporting README/assets. Until that listing is
...
The installed plugin registers under the short slug `xiaozhi` so tool and command names
...
- XIAOZHI is moving to a skills-first workflow surface. Legacy `commands/` remain for
