...
XIAOZHI keeps `.mcp.json` at the repository root for Codex plugin installs and manual MCP setup.
...
The Claude plugin slug is intentionally short (`xiaozhi`), but this opt-out is still required because legacy installs and strict provider gateways have failed on generated names from longer plugin identifiers.
...
This explicit empty object prevents Claude plugin installs from auto-loading XIAOZHI's root MCP definitions.
