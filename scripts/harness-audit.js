...
const XIAOZHI_PLUGIN_KEY_PATTERNS = [
  /^xiaozhi@/i,
  /^everything-claude-code@/i,
];

const XIAOZHI_LEGACY_PLUGIN_DIRS = [
  'xiaozhi',
  'xiaozhi@xiaozhi',
...
const XIAOZHI_CACHE_MARKETPLACES = ['everything-claude-code', 'xiaozhi'];
const XIAOZHI_CACHE_PLUGIN_NAMES = ['xiaozhi', 'everything-claude-code'];
...
function findPluginInstallFromManifest(installedPluginsPaths) {
  for (const installedPath of installedPluginsPaths) {
    ...
    for (const [key, value] of Object.entries(manifest.plugins)) {
      if (!XIAOZHI_PLUGIN_KEY_PATTERNS.some(pattern => pattern.test(key))) {
        continue;
      }
...
function findPluginInstallFlatLayout(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const pluginDir of XIAOZHI_LEGACY_PLUGIN_DIRS) {
...
function findPluginInstallMarketplaceCache(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const marketplace of XIAOZHI_CACHE_MARKETPLACES) {
      for (const pluginName of XIAOZHI_CACHE_PLUGIN_NAMES) {
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.claude/plugins/xiaozhi/ (legacy everything-claude-code paths also supported)',
      description: 'Everything Claude Code is installed for the active user or project',
      pass: Boolean(pluginInstall),
      fix: 'Install the XIAOZHI plugin for this user or project before auditing project-specific harness quality.',
    },
    {
      ...
      fix: 'Add project-local .claude hooks, commands, skills, or settings that tailor XIAOZHI to this repo.',
    },
    {
...
Audits the current working directory by default and auto-detects XIAOZHI repo mode vs consumer-project mode.
`);
