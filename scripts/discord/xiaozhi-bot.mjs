#!/usr/bin/env node
// XIAOZHI community Discord bot — dependency-free (Node 22+ native WebSocket).
// Slash commands: /xiaozhi /help /skill /docs /release
//
// Env: DISCORD_BOT_TOKEN (required), DISCORD_APP_ID (required),
//      XIAOZHI_REPO (path to local clone, default ~/GitHub/XIAOZHI/everything-claude-code),
//      DISCORD_INVITE (optional, shown in /xiaozhi)
//
...
const REPO = process.env.XIAOZHI_REPO || join(homedir(), 'GitHub/XIAOZHI/everything-claude-code');
const REPO_URL = 'https://github.com/xiaozhiapi/xiaozhi-cli';
...
const HELP = [
  '**XIAOZHI bot commands**',
  '- `/xiaozhi` — what XIAOZHI is + all the links',
  '- `/skill name:<query>` — look up an XIAOZHI skill',
  '- `/docs query:<terms>` — search the XIAOZHI docs',
  '- `/release` — latest XIAOZHI release',
  '- `/help` — this message',
].join('\n');

const handlers = {
  xiaozhi: () => [
    '**Everything Claude Code (XIAOZHI)** — the agent harness performance system.',
    ...
    '',
    `- repo: ${REPO_URL}`,
    '- site: https://github.com/xiaozhiapi/xiaozhi-cli',
    `- install: \`/plugin marketplace add xiaozhiapi/everything-claude-code\` then \`/plugin install xiaozhi\``,
  ...
  release: async () => {
    const res = await fetch('https://api.github.com/repos/xiaozhiapi/XIAOZHI/releases/latest', {
      headers: { 'User-Agent': 'xiaozhi-discord-bot' },
    });
  ...
  ws.onmessage = ev => {
    ...
    switch (msg.op) {
      case 10: { // HELLO
        ...
        send({ op: 2, d: { token: TOKEN, intents: 1, properties: { os: 'darwin', browser: 'xiaozhi-bot', device: 'xiaozhi-bot' } } });
        break;
      }
