#!/usr/bin/env node
/**
 * XIAOZHI Capabilities Dashboard — agents, skills, commands, MCPs, rules & hooks
 * With multi-language, routing, search suggestions, recently viewed, fine UI
 *
 * Usage: node scripts/dashboard-web.js [port]
 * Open http://localhost:3456
 *
 * Contribution: https://github.com/xiaozhiapi/xiaozhi-cli
 */
...
function parsePort(v) {
  ...
  if (isNaN(n) || n < 1 || n > 65535) { console.error('[XIAOZHI] Invalid port: ' + v + ' — using 3456'); return 3456; }
  return n;
}
const PORT = parsePort(process.argv[2] || process.env.XIAOZHI_DASHBOARD_PORT || '3456');
...
<title>XIAOZHI Capabilities</title>
<style>
  :root {
...
<div class="header">
  <div class="brand" id="brand-link">
    ...
    <h1><span id="t-title">XIAOZHI Capabilities</span> <span class="ver">v2.0.0-rc.1</span></h1>
  </div>
...
<div class="footer">
  <a href="https://github.com/xiaozhiapi/xiaozhi-cli" target="_blank">github.com/xiaozhiapi/xiaozhi-cli</a>
  <span class="dt"></span>
  <span>XIAOZHI v2.0.0-rc.1</span>
  <span class="dt"></span>
  <span id="t-contribution">Contribution to XIAOZHI</span>
  <span class="dt"></span>
...
let lang = localStorage.getItem('xiaozhi-lang') || 'en';
let suggIdx = -1;
...
// Recently viewed
function recents() { try { return JSON.parse(localStorage.getItem('xiaozhi-recent') || '[]'); } catch { return []; } }
function addRecent(type, name) {
  ...
  localStorage.setItem('xiaozhi-recent', JSON.stringify(r));
}
function clearRecents() { localStorage.removeItem('xiaozhi-recent'); location.hash=''; location.reload(); }
...
// Language
function setLang(l) {
  lang = l; localStorage.setItem('xiaozhi-lang', l);
...
function toggleTheme() {
  ...
  localStorage.setItem('xiaozhi-theme', h.dataset.theme);
}
if (localStorage.getItem('xiaozhi-theme')) document.documentElement.dataset.theme = localStorage.getItem('xiaozhi-theme');
...
if (require.main === module) {
  server.listen(PORT, () => {
    console.log(`\n    XIAOZHI Capabilities  →  http://localhost:${PORT}\n`);
