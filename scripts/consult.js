...
const MAX_LIMIT = 20;
const SCHEMA_VERSION = 'xiaozhi.consult.v1';
...
function showHelp(exitCode = 0) {
  console.log(`
Consult XIAOZHI install components and profiles from any project
...
function commandFor(kind, id, target) {
  if (kind === 'profile') {
    return `npx xiaozhi install --profile ${id} --target ${target}`;
  }

  return `npx xiaozhi install --profile minimal --target ${target} --with ${id}`;
}

function planCommandFor(componentId, target) {
  return `npx xiaozhi plan --profile minimal --target ${target} --with ${componentId}`;
}
...
function buildConsultation(options) {
  ...
  return {
    ...
    nextSteps: matches.length > 0
      ...
      : [
        'Run `npx xiaozhi catalog components` to browse all components.',
...
function formatText(payload) {
  const lines = [
    `XIAOZHI consult (${payload.generatedAt})`,
    `Query: ${payload.query}`,
  ...
  if (payload.matches.length === 0) {
    ...
    lines.push('Try: npx xiaozhi catalog components');
  } else {
