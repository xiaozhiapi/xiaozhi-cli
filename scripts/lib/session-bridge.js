...
/**
 * Shared session bridge utilities for XIAOZHI hooks.
 *
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `xiaozhi-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `xiaozhi-metrics-bridge` and the background
 * `xiaozhi-statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/xiaozhi-metrics-bridge.js` (commit 9b1d8918) for the
...
function resolveSessionId() {
  const raw = process.env.XIAOZHI_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
