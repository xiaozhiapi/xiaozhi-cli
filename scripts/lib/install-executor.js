...
const CLAUDE_XIAOZHI_NAMESPACE = 'xiaozhi';
...
function addCursorAgentDataScaffoldOperations(operations, options) {
  ...
  addFileCopyOperation(operations, {
    ...
    sourceRelativePath: path.join('scaffolds', 'cursor', 'xiaozhi-agent-data.json'),
    destinationPath: path.join(options.targetRoot, 'xiaozhi-agent-data.json'),
  ...
  addFileCopyOperation(operations, {
    moduleId: options.moduleId,
    sourceRoot: options.sourceRoot,
    sourceRelativePath: path.join('scaffolds', 'cursor', 'rules', 'xiaozhi-agent-data-home.mdc'),
    destinationPath: path.join(options.targetRoot, 'rules', 'xiaozhi-agent-data-home.mdc'),
    strategy: 'preserve-relative-path',
  });
...
function planClaudeStyleLegacyInstall(context, { adapterId, adapterRootInput, rulesDir: rulesDirOverride }) {
  ...
  const rulesDir = rulesDirOverride || path.join(targetRoot, 'rules', CLAUDE_XIAOZHI_NAMESPACE);
