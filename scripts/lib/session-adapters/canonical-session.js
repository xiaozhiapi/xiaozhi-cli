...
const SESSION_SCHEMA_VERSION = 'xiaozhi.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'xiaozhi.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'xiaozhi-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.XIAOZHI_SESSION_RECORDING_DIR === 'string' && process.env.XIAOZHI_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.XIAOZHI_SESSION_RECORDING_DIR);
  }
