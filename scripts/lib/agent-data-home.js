/**
 * Resolve XIAOZHI agent data home (memory persistence root) across harnesses.
 *
 ...
 * purpose, consistent with XIAOZHI script modules elsewhere. Automated PR reviewers
 ...
 *
 * @see https://github.com/xiaozhiapi/xiaozhi-cli/issues/2065
 */

const fs = require('fs');
const path = require('path');

const AGENT_DATA_HOME_ENV = 'XIAOZHI_AGENT_DATA_HOME';
const DEFAULT_CLAUDE_DIR_NAME = '.claude';
const DEFAULT_CURSOR_XIAOZHI_DIR_SEGMENTS = ['.cursor', 'xiaozhi'];
const PROJECT_CONFIG_RELATIVE = path.join('.cursor', 'xiaozhi-agent-data.json');
 ...
 * dependency and risk divergent defaults for `~/.cursor/xiaozhi` vs `~/.claude`.
 *
...
/**
 * Project root for a config file under .cursor/xiaozhi-agent-data.json.
 */
...
function getDefaultCursorAgentDataHome() {
  return path.join(getHomeDirFromEnv(), ...DEFAULT_CURSOR_XIAOZHI_DIR_SEGMENTS);
}
...
function readProjectConfigAt(configPath) {
  ...
  try {
    ...
    const candidate = parsed.agentDataHome || parsed.XIAOZHI_AGENT_DATA_HOME;
    if (typeof candidate !== 'string' || !candidate.trim()) return null;
    const projectRoot = resolveProjectRootFromConfigPath(configPath);
    return expandHomePath(candidate, projectRoot);
  } catch (error) {
    console.error(
      `[XIAOZHI] Failed to read or parse agent data config at ${configPath}: ${error.message}`
    );
    return null;
  }
}
...
/**
 * Set XIAOZHI_AGENT_DATA_HOME on the current process when unset (hook subprocess safety net).
...
function getCursorSessionEnvPayload(options = {}) {
  ...
  return {
    XIAOZHI_AGENT_DATA_HOME: agentDataHome,
  };
}
...
module.exports = {
  ...
  DEFAULT_CURSOR_XIAOZHI_DIR_SEGMENTS,
