...
Pull the latest XIAOZHI repo changes and reinstall the current context's managed targets
...
function deriveRepoRootFromState(state) {
  ...
  throw new Error('Unable to infer XIAOZHI repo root from install-state operations');
}
...
function validateRepoRoot(repoRoot) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Invalid XIAOZHI repo root: missing package.json at ${packageJsonPath}`);
  }

  if (!fs.existsSync(installApplyPath)) {
    throw new Error(`Invalid XIAOZHI repo root: missing install script at ${installApplyPath}`);
  }
...
function runAutoUpdate(options = {}, dependencies = {}) {
  ...
  if (!requestedRepoRoot) {
    ...
    if (uniqueRepoRoots.length > 1) {
      throw new Error(`Multiple XIAOZHI repo roots detected: ${uniqueRepoRoots.join(', ')}`);
    }
  }
...
function printHuman(result) {
  if (result.results.length === 0) {
    console.log('No XIAOZHI install-state files found for the current home/project context.');
    return;
  }
