#!/usr/bin/env node
/**
 * XIAOZHI Metrics Bridge — PostToolUse hook
 *
 * Maintains a running session aggregate in /tmp/xiaozhi-metrics-{session}.json.
 * This bridge file is read by xiaozhi-statusline.js and xiaozhi-context-monitor.js,
...
const WARNING_CACHE_PREFIX = 'xiaozhi-metrics-cost-warnings-';
  ...
  try {
    ...
    if (malformed > 0) {
      writeCostWarningIfChanged(
        ...
        `[xiaozhi-metrics-bridge] skipped ${malformed} malformed line(s) in ${costsPath}\n`
      );
    }
    ...
    if (err && err.code !== 'ENOENT') {
      writeCostWarningIfChanged(
        ...
        `[xiaozhi-metrics-bridge] failing open after ${err.name || 'error'} reading ${costsPath}: ${err.message || String(err)}\n`
      );
    }
...
function run(rawInput) {
  try {
    ...
    const sessionId = sanitizeSessionId(input.session_id) || sanitizeSessionId(process.env.XIAOZHI_SESSION_ID) || sanitizeSessionId(process.env.CLAUDE_SESSION_ID);
