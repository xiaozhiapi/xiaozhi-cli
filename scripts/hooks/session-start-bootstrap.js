 ...
 *
 * Bootstrap loader for the XIAOZHI SessionStart hook.
 *
 ...
 *   2. Resolves the XIAOZHI plugin root directory (via CLAUDE_PLUGIN_ROOT env var
...
const CURRENT_PLUGIN_SLUG = 'xiaozhi';
...
/**
 * Returns true when `candidate` looks like a valid XIAOZHI plugin root, i.e. the
...
/**
 * Resolves the XIAOZHI plugin root using the following priority order:
 ...
 *   4. Versioned cache directories under ~/.claude/plugins/cache/{xiaozhi,everything-claude-code}/
...
function resolvePluginRoot() {
  ...
  // Walk versioned cache: ~/.claude/plugins/cache/{xiaozhi,everything-claude-code}/<org>/<version>/
  try {
...
process.stderr.write(
  '[SessionStart] WARNING: could not resolve XIAOZHI plugin root; skipping session-start hook\n'
);
process.stdout.write(raw);
