#!/usr/bin/env node
/**
 * XIAOZHI Context Monitor — PostToolUse hook
 *
 * Reads bridge file from xiaozhi-metrics-bridge.js and injects agent-facing
...
function costWarningsEnabled(env = process.env) {
  return isEnabledEnv(env.XIAOZHI_CONTEXT_MONITOR_COST_WARNINGS, true);
}
...
function getWarnPath(sessionId) {
  return path.join(os.tmpdir(), `xiaozhi-ctx-warn-${sessionId}.json`);
}
 ...
 * and `writeCostWarningIfChanged` in `scripts/hooks/xiaozhi-metrics-bridge.js`.
 *
 * @param {string} sessionId
 * @param {object} state
 */
