...
function getPluginRoot(options = {}) {
  ...
  if (process.env.XIAOZHI_PLUGIN_ROOT && process.env.XIAOZHI_PLUGIN_ROOT.trim()) {
    return process.env.XIAOZHI_PLUGIN_ROOT.trim();
  }
...
function getPhaseFromHookId(hookId) {
  const prefix = String(hookId || process.env.XIAOZHI_HOOK_ID || '').split(':')[0];
  return prefix === 'pre' || prefix === 'post' ? prefix : null;
}

function getTimeoutMs() {
  const parsed = Number.parseInt(process.env.XIAOZHI_OBSERVE_RUNNER_TIMEOUT_MS || '', 10);
...
function run(raw, options = {}) {
  ...
  const result = spawnSync(shell, [toShellPath(observePath), phase], {
    ...
    env: {
      ...
      XIAOZHI_PLUGIN_ROOT: pluginRoot
    },
    cwd: process.cwd(),
...
if (require.main === module) {
  ...
  const output = run(raw, { hookId: process.argv[2] || process.env.XIAOZHI_HOOK_ID });
