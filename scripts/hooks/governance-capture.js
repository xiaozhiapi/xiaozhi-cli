 ...
 *
 * Enable: Set XIAOZHI_GOVERNANCE_CAPTURE=1
 * Configure session: Set XIAOZHI_SESSION_ID for session correlation
 */
...
function run(rawInput, options = {}) {
  ...
  if (String(process.env.XIAOZHI_GOVERNANCE_CAPTURE || '').toLowerCase() !== '1') {
    return rawInput;
  }

  const sessionId = process.env.XIAOZHI_SESSION_ID || null;
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    const result = run(raw, {
      truncated,
      maxStdin: Number(process.env.XIAOZHI_HOOK_INPUT_MAX_BYTES) || MAX_STDIN,
    });
