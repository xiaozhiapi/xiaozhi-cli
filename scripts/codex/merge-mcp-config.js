...
/**
 * Merge XIAOZHI-recommended MCP servers into a Codex config.toml.
 *
 ...
 *   - Log warnings when an existing server's config differs from the XIAOZHI recommendation.
 *   - With --update-mcp, also replace existing XIAOZHI-managed servers.
 *
...
try {
  ...
  console.error('[xiaozhi-mcp] Missing dependency: @iarna/toml');
  console.error('[xiaozhi-mcp] Run: npm install   (from the XIAOZHI repo root)');
  process.exit(1);
}
...
// ---------------------------------------------------------------------------
// XIAOZHI-recommended MCP servers
// ---------------------------------------------------------------------------
...
// XIAOZHI <= 2.0.0 emitted [mcp_servers.exa] with a `url` key. Codex rejects
// `url` for stdio servers, which makes the *entire* config.toml fail to
// load (#2224). Repair exactly that XIAOZHI-emitted form on every merge so
...
// Legacy section names that should be treated as an existing XIAOZHI server.
...
function log(msg) {
  console.log(`[xiaozhi-mcp] ${msg}`);
}

function warn(msg) {
  console.warn(`[xiaozhi-mcp] WARNING: ${msg}`);
}
...
function main() {
  ...
  const disabledServers = new Set(parseDisabledMcpServers(process.env.XIAOZHI_DISABLED_MCPS));
  ...
  if (!fs.existsSync(configPath)) {
    console.error(`[xiaozhi-mcp] Config file not found: ${configPath}`);
    process.exit(1);
  }

  log(`Package manager: ${PM_NAME} (exec: ${PM_EXEC})`);
  if (disabledServers.size > 0) {
    log(`Disabled via XIAOZHI_DISABLED_MCPS: ${[...disabledServers].join(', ')}`);
  }
  ...
  try {
    ...
    console.error(`[xiaozhi-mcp] Failed to parse ${configPath}: ${err.message}`);
    process.exit(1);
  }
  ...
  // Repair schema-invalid entries emitted by earlier XIAOZHI versions (#2224).
  for (const [name, invalidUrl] of Object.entries(RETIRED_INVALID_URL_SERVERS)) {
    ...
    if (isBrokenEccForm) {
      toRemoveLog.push(`mcp_servers.${name} (invalid url entry from earlier XIAOZHI versions)`);
      raw = removeServerFromText(raw, name, existing);
      log(`  [repair] mcp_servers.${name} — url is not valid for Codex stdio servers, removing`);
    }
  }

  for (const [name, spec] of Object.entries(XIAOZHI_SERVERS)) {
    const entry = existing[name];
    ...
    // (Codex itself rejects url-form stdio servers; XIAOZHI only ever emits
    ...
    if (finalEntry) {
      if (updateMcp) {
        ...
        if (legacyName && !hasCanonical) {
          ...
          warn(`mcp_servers.${name} differs from XIAOZHI recommendation (run with --update-mcp to refresh)`);
        } else {
  ...
  if (toAppend.length === 0 && !hasRemovals) {
    log('All XIAOZHI MCP servers already present. Nothing to do.');
    return;
  }
