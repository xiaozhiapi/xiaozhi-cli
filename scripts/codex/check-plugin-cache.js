...
function usage() {
  console.log([
    ...
    '  --marketplace <name> Marketplace cache name (default: xiaozhi)',
    '  --plugin <name>      Plugin cache name (default: xiaozhi)',
...
function log(message) {
  console.log(`[xiaozhi-codex] ${message}`);
}
...
function checkCache(options) {
  ...
  if (!fs.existsSync(manifestPath)) {
    ...
    if (versions.length > 0) {
      ...
      if (options.marketplace === 'xiaozhi' && options.plugin === 'xiaozhi') {
        log('Run: codex plugin marketplace add xiaozhiapi/XIAOZHI');
      } else {
  ...
  if (failures > 0) {
    ...
    const syncScript = path.join(REPO_ROOT, 'scripts', 'sync-xiaozhi-to-codex.sh');
    if (fs.existsSync(syncScript)) {
      log('Use the supported sync path until the cache contains the referenced files:');
      log('npm install && bash scripts/sync-xiaozhi-to-codex.sh');
    } else {
      log('Use the supported manual sync workflow from your XIAOZHI installation.');
    }
    return 1;
  }
...
function main() {
  ...
  try {
    ...
    console.error(`[xiaozhi-codex] ${error.message}`);
    usage();
    process.exit(1);
  }
