...
# XIAOZHI Codex global regression sanity check.
# Validates that global ~/.codex state matches expected XIAOZHI integration.
...
HOOKS_DIR_EXPECT="${XIAOZHI_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}"
...
printf 'XIAOZHI GLOBAL SANITY CHECK\n'
printf 'Repo: %s\n' "$REPO_ROOT"
printf 'Codex home: %s\n\n' "$CODEX_HOME"
...
if [[ -f "$AGENTS_FILE" ]]; then
  if search_file '^# Everything Claude Code \(XIAOZHI\)' "$AGENTS_FILE"; then
    ok "AGENTS contains XIAOZHI root instructions"
  else
    fail "AGENTS missing XIAOZHI root instructions"
  fi

  if search_file '^# Codex Supplement \(From XIAOZHI \.codex/AGENTS\.md\)' "$AGENTS_FILE"; then
    ok "AGENTS contains XIAOZHI Codex supplement"
  else
    fail "AGENTS missing XIAOZHI Codex supplement"
  fi
fi
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  # XIAOZHI <= 2.0.0 emitted a url-only exa entry that Codex's stdio-only
  ...
  if search_file '^\[mcp_servers\.exa\]' "$CONFIG_FILE"; then
    ...
    if printf '%s\n' "$exa_block" | grep -Eq '^[[:space:]]*url[[:space:]]*=' \
      ...
      fail "MCP section [mcp_servers.exa] uses a url key, which Codex rejects for stdio servers — re-run xiaozhi-sync-codex to repair (#2224)"
    else
...
if [[ -d "$SKILLS_DIR" ]]; then
  ...
  if [[ "$missing_skills" -eq 0 ]]; then
    ok "All 16 XIAOZHI skills are present in $SKILLS_DIR"
  else
    warn "$missing_skills XIAOZHI skills missing from $SKILLS_DIR (install via XIAOZHI installer or npx skills)"
  fi
else
  warn "Skills directory missing ($SKILLS_DIR) — install via XIAOZHI installer or npx skills"
fi

if [[ -f "$PROMPTS_DIR/xiaozhi-prompts-manifest.txt" ]]; then
  ok "Command prompts manifest exists"
else
  fail "Command prompts manifest missing"
fi

if [[ -f "$PROMPTS_DIR/xiaozhi-extension-prompts-manifest.txt" ]]; then
  ok "Extension prompts manifest exists"
else
  fail "Extension prompts manifest missing"
fi

command_prompts_count="$(find "$PROMPTS_DIR" -maxdepth 1 -type f -name 'xiaozhi-*.md' 2>/dev/null | wc -l | tr -d ' ')"
if [[ "$command_prompts_count" -ge 43 ]]; then
  ok "XIAOZHI prompts count is $command_prompts_count (expected >= 43)"
else
  fail "XIAOZHI prompts count is $command_prompts_count (expected >= 43)"
fi
...
if command -v xiaozhi-sync-codex >/dev/null 2>&1; then
  ok "xiaozhi-sync-codex command is in PATH"
else
  warn "xiaozhi-sync-codex is not in PATH"
fi

if command -v xiaozhi-install-git-hooks >/dev/null 2>&1; then
  ok "xiaozhi-install-git-hooks command is in PATH"
else
  warn "xiaozhi-install-git-hooks is not in PATH"
fi

if command -v xiaozhi-check-codex >/dev/null 2>&1; then
  ok "xiaozhi-check-codex command is in PATH"
else
  warn "xiaozhi-check-codex is not in PATH (this is expected before alias setup)"
fi

printf '\nSummary: checks=%d, warnings=%d, failures=%d\n' "$checks" "$warnings" "$failures"
if [[ "$failures" -eq 0 ]]; then
  printf 'XIAOZHI GLOBAL SANITY: PASS\n'
else
  printf 'XIAOZHI GLOBAL SANITY: FAIL\n'
  exit 1
fi
