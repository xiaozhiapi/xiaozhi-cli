...
try {
  ...
  console.error('[xiaozhi-codex] Missing dependency: @iarna/toml');
  console.error('[xiaozhi-codex] Run: npm install   (from the XIAOZHI repo root)');
  process.exit(1);
}
...
function warn(message) {
  console.warn(`[xiaozhi-codex] WARNING: ${message}`);
}
...
function main() {
  ...
  if (!fs.existsSync(referencePath)) {
    console.error(`[xiaozhi-codex] Reference config not found: ${referencePath}`);
    process.exit(1);
  }

  if (!fs.existsSync(configPath)) {
    console.error(`[xiaozhi-codex] Config file not found: ${configPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[xiaozhi-codex] Failed to parse TOML: ${error.message}`);
    process.exit(1);
  }
