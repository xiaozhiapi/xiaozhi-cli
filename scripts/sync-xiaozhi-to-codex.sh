...
# Sync Everything Claude Code (XIAOZHI) assets into a local Codex CLI setup.
# - Backs up ~/.codex config and AGENTS.md
# - Merges XIAOZHI AGENTS.md into existing AGENTS.md (marker-based, preserves user content)
...
# - Merges XIAOZHI MCP servers into config.toml (add-only via Node TOML parser)
...
BACKUP_DIR="$CODEX_HOME/backups/xiaozhi-$STAMP"

log() { printf '[xiaozhi-sync] %s\n' "$*"; }
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# XIAOZHI Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the XIAOZHI `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
require_path "$REPO_ROOT/AGENTS.md" "XIAOZHI AGENTS.md"
require_path "$AGENTS_CODEX_SUPP_SRC" "XIAOZHI Codex AGENTS supplement"
require_path "$CODEX_AGENTS_SRC" "XIAOZHI Codex agent roles"
require_path "$PROMPTS_SRC" "XIAOZHI commands directory"
require_path "$BASELINE_MERGE_SCRIPT" "XIAOZHI Codex baseline merge script"
require_path "$HOOKS_INSTALLER" "XIAOZHI global git hooks installer"
require_path "$SANITY_CHECKER" "XIAOZHI global sanity checker"
require_path "$CURSOR_RULES_DIR" "XIAOZHI Cursor rules directory"
require_path "$CONFIG_FILE" "Codex config.toml"
require_path "$MCP_MERGE_SCRIPT" "XIAOZHI MCP merge script"
...
XIAOZHI_BEGIN_MARKER="<!-- BEGIN XIAOZHI -->"
XIAOZHI_END_MARKER="<!-- END XIAOZHI -->"

compose_xiaozhi_block() {
  printf '%s\n' "$XIAOZHI_BEGIN_MARKER"
  cat "$AGENTS_ROOT_SRC"
  printf '\n\n---\n\n'
  printf '# Codex Supplement (From XIAOZHI .codex/AGENTS.md)\n\n'
  cat "$AGENTS_CODEX_SUPP_SRC"
  printf '\n%s\n' "$XIAOZHI_END_MARKER"
}

log "Merging XIAOZHI AGENTS into $AGENTS_FILE (preserving user content)"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] merge XIAOZHI block into %s from %s + %s\n' "$AGENTS_FILE" "$AGENTS_ROOT_SRC" "$AGENTS_CODEX_SUPP_SRC"
else
  replace_xiaozhi_section() {
    # Replace the XIAOZHI block between markers in $AGENTS_FILE with fresh content.
    # Uses awk to correctly handle all positions including line 1.
    local tmp
    tmp="$(mktemp)"
    local xiaozhi_tmp
    xiaozhi_tmp="$(mktemp)"
    compose_xiaozhi_block > "$xiaozhi_tmp"
    awk -v begin="$XIAOZHI_BEGIN_MARKER" -v end="$XIAOZHI_END_MARKER" -v xiaozhi="$xiaozhi_tmp" '
      { gsub(/\r$/, "") }
      $0 == begin { skip = 1; while ((getline line < xiaozhi) > 0) print line; close(xiaozhi); next }
      $0 == end   { skip = 0; next }
      !skip        { print }
    ' "$AGENTS_FILE" > "$tmp"
    # Write through the path (preserves symlinks) instead of mv
    cat "$tmp" > "$AGENTS_FILE"
    rm -f "$tmp" "$xiaozhi_tmp"
  }

  if [[ ! -f "$AGENTS_FILE" ]]; then
    # No existing file — create fresh with markers
    compose_xiaozhi_block > "$AGENTS_FILE"
  elif awk -v b="$XIAOZHI_BEGIN_MARKER" -v e="$XIAOZHI_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++; if (!fb) fb = NR }
        $0 == e { ec++; if (!fe) fe = NR }
        END { exit !(bc == 1 && ec == 1 && fb < fe) }
      ' "$AGENTS_FILE"; then
    # Exactly one BEGIN/END pair in correct order — replace only the XIAOZHI section
    replace_xiaozhi_section
  elif awk -v b="$XIAOZHI_BEGIN_MARKER" -v e="$XIAOZHI_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++ } $0 == e { ec++ }
    ...
    log "WARNING: XIAOZHI markers found but not a clean pair — stripping markers and re-appending"
    _fix_tmp="$(mktemp)"
    awk -v b="$XIAOZHI_BEGIN_MARKER" -v e="$XIAOZHI_END_MARKER" '
      { gsub(/\r$/, "") }
      $0 == b { skip = 1; next }
      $0 == e { skip = 0; next }
      !skip   { print }
    ...
    # Existing file without markers — append XIAOZHI block, preserving existing content.
    # Legacy XIAOZHI-only files will have duplicate content after this first run, but
    ...
    log "No XIAOZHI markers found — appending managed block (backup saved)"
    {
      printf '\n\n'
      compose_xiaozhi_block
    } >> "$AGENTS_FILE"
...
log "Merging XIAOZHI Codex baseline into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
# ~/.agents/skills/ (installed by XIAOZHI installer / npx skills).
# Copying into ~/.codex/skills/ was unnecessary.

log "Generating prompt files from XIAOZHI commands"
run_or_echo mkdir -p "$PROMPTS_DEST"
manifest="$PROMPTS_DEST/xiaozhi-prompts-manifest.txt"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] > %s\n' "$manifest"
else
  : > "$manifest"
...
while IFS= read -r -d '' command_file; do
  ...
  out="$PROMPTS_DEST/xiaozhi-$name.md"
  if [[ "$MODE" == "dry-run" ]]; then
...
extension_manifest="$PROMPTS_DEST/xiaozhi-extension-prompts-manifest.txt"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] > %s\n' "$extension_manifest"
else
  : > "$extension_manifest"
...
write_extension_prompt "xiaozhi-tool-run-tests.md" <<EOF
# XIAOZHI Tool Prompt: run-tests
...
write_extension_prompt "xiaozhi-tool-check-coverage.md" <<EOF
# XIAOZHI Tool Prompt: check-coverage
...
write_extension_prompt "xiaozhi-tool-security-audit.md" <<EOF
# XIAOZHI Tool Prompt: security-audit
...
write_extension_prompt "xiaozhi-rules-pack-common.md" <<EOF
# XIAOZHI Rule Pack: common (optional)

Apply XIAOZHI common engineering rules for this session. Use these files as the source of truth:
...
write_extension_prompt "xiaozhi-rules-pack-typescript.md" <<EOF
# XIAOZHI Rule Pack: typescript (optional)

Apply XIAOZHI common rules plus TypeScript-specific rules for this session.

## Common
Use \`$PROMPTS_DEST/xiaozhi-rules-pack-common.md\`.
...
write_extension_prompt "xiaozhi-rules-pack-python.md" <<EOF
# XIAOZHI Rule Pack: python (optional)

Apply XIAOZHI common rules plus Python-specific rules for this session.
...
write_extension_prompt "xiaozhi-rules-pack-golang.md" <<EOF
# XIAOZHI Rule Pack: golang (optional)

Apply XIAOZHI common rules plus Go-specific rules for this session.
...
write_extension_prompt "xiaozhi-rules-pack-swift.md" <<EOF
# XIAOZHI Rule Pack: swift (optional)

Apply XIAOZHI common rules plus Swift-specific rules for this session.
...
log "Merging XIAOZHI MCP servers into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  XIAOZHI_GLOBAL_HOOKS_DIR="${XIAOZHI_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
