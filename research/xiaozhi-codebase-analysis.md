...
`dashboard.rs:495` — `new_session()` logs `"New session dialog requested"` but does nothing. Users must use the CLI (`xiaozhi start --task "..."`) to create sessions; the TUI dashboard cannot.
...
1. **Add environment variable support to `Config::load()`** — `XIAOZHI_DB_PATH`, `XIAOZHI_WORKTREE_ROOT`, `XIAOZHI_DEFAULT_AGENT`. Standard practice for CLI tools.
