...
- Discord community launch: server + GitHub PR/issue/release feed, `release-announce.yml` workflow (announce + pin + Discussions cross-post), and a dependency-free community bot (`scripts/discord/xiaozhi-bot.mjs`) with `/xiaozhi`, `/help`, `/skill`, `/docs`, `/release`.
- `orch-*` orchestrator skill family and dynamic workflow team orchestration.
- `kubernetes-patterns` skill, worktree-lifecycle service, MCP inventory (`xiaozhi.mcp.v1`), codex-worktree and opencode session adapters.
...
- Adds the public XIAOZHI 2.0 release-candidate surface for the Hermes operator story.
- Documents XIAOZHI as the reusable cross-harness substrate across Claude Code, Codex, Cursor, OpenCode, and Gemini.
...
- This is a release candidate, not a GA claim for the full XIAOZHI 2.0 control-plane roadmap.
...
- This release is a repo-surface correction and ecosystem sync, not a claim that the full XIAOZHI 2.0 roadmap is complete.
...
- `everything-claude-code` — Self-referential XIAOZHI skill (#335)
...
- Added `XIAOZHI_HOOK_PROFILE` and `XIAOZHI_DISABLED_HOOKS` controls.
