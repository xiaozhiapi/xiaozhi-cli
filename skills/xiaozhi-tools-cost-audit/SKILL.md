---
name: xiaozhi-tools-cost-audit
description: Evidence-first XIAOZHI Tools burn and billing audit workflow. Use when investigating runaway PR creation, quota bypass, premium-model leakage, duplicate jobs, or GitHub App cost spikes in the XIAOZHI Tools repo.
metadata:
  origin: XIAOZHI
---
...
Use this skill when the user suspects the XIAOZHI Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [XIAOZHI-Tools](../../XIAOZHI-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.

## Skill Stack

Pull these XIAOZHI-native skills into the workflow when relevant:
...
- user says XIAOZHI Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `XIAOZHI-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `XIAOZHI-Tools` repo, not in `everything-claude-code`
...
- switch into the sibling `XIAOZHI-Tools` repo
