...
Prefer dry-run plans and local queue files before enabling recurring or event-driven actions. Keep credentials, private workspace exports, personal datasets, and account-specific automations out of reusable XIAOZHI artifacts.
...
mcp__scheduled-tasks__create_scheduled_task({
  ...
  prompt: "Review all open PRs in xiaozhiapi/everything-claude-code. For each: check CI status, review changes, flag issues. Post summary to memory.",
...
| Hermes Component | XIAOZHI Equivalent | How |
|------------------|---------------|-----|
...
| Orchestration | XIAOZHI skills + agents | Skill definitions direct agent behavior |
| Computer Use | computer-use MCP | Native browser and desktop control |
| Context Manager | Session management + memory | XIAOZHI 2.0 session lifecycle |
