---
name: xiaozhi-guide
description: Guide users through XIAOZHI's current agents, skills, commands, hooks, rules, install profiles, and project onboarding by reading the live repository surface before answering.
metadata:
  origin: community
---
...
Answer from current files, not memory. XIAOZHI changes quickly, so hard-coded catalog counts, feature lists, and install instructions go stale.

When the XIAOZHI repository is available, inspect the relevant files before giving a concrete answer:
...
Use `/project-init` when the user wants XIAOZHI configured for a target repo. The expected sequence is:
