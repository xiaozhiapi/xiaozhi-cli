...
| Feature | v2.0 | v2.1 |
|---------|------|------|
| Storage | Global (`~/.claude/homunculus/`) | Project-scoped (`${XDG_DATA_HOME:-~/.local/share}/xiaozhi-homunculus/projects/<hash>/`) |
...
Each project gets a 12-character hash ID (e.g., `a1b2c3d4e5f6`). A registry file at `${XDG_DATA_HOME:-~/.local/share}/xiaozhi-homunculus/projects.json` maps IDs to human-readable names.
...
2. `$XDG_DATA_HOME/xiaozhi-homunculus`
...
```bash
# Global directories
mkdir -p "${XDG_DATA_HOME:-$HOME/.local/share}/xiaozhi-homunculus"/{instincts/{personal,inherited},evolved/{agents,skills,commands},projects}
...
```
${XDG_DATA_HOME:-~/.local/share}/xiaozhi-homunculus/
...
- [XIAOZHI-Tools GitHub App](https://github.com/xiaozhiapi/xiaozhi-cli) - Generate instincts from repo history
