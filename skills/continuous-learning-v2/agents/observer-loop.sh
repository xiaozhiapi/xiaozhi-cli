...
ANALYSIS_COOLDOWN="${XIAOZHI_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${XIAOZHI_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${XIAOZHI_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set XIAOZHI_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${XIAOZHI_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  observer_tmp_dir="${PROJECT_DIR}/.observer-tmp"
  mkdir -p "$observer_tmp_dir"
  analysis_file="$(mktemp "${observer_tmp_dir}/xiaozhi-observer-analysis.XXXXXX.jsonl")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/xiaozhi-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${XIAOZHI_OBSERVER_TIMEOUT_SECONDS:-120}"
  ...
  if [ -n "${XIAOZHI_OBSERVER_MAX_TURNS:-}" ]; then
    max_turns="${XIAOZHI_OBSERVER_MAX_TURNS}"
  else
  ...
  # but an explicit XIAOZHI_OBSERVER_MAX_TURNS override may be non-numeric, empty, or too
  ...
  XIAOZHI_SKIP_OBSERVE=1 XIAOZHI_HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print \
