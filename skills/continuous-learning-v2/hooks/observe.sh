...
CONFIG_DIR="$(_xiaozhi_resolve_homunculus_dir)"
...
#   - XIAOZHI observing its own Haiku observer sessions (self-loop)
#   - XIAOZHI observing other tools' automated sessions
...
# (XIAOZHI_HOOK_PROFILE=minimal, XIAOZHI_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${XIAOZHI_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${XIAOZHI_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_XIAOZHI_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_XIAOZHI_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_XIAOZHI_SKIP_PATHS="${XIAOZHI_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _XIAOZHI_SKIP_ARRAY <<< "$_XIAOZHI_SKIP_PATHS"
  for _pattern in "${_XIAOZHI_SKIP_ARRAY[@]}"; do
...
SIGNAL_EVERY_N="${XIAOZHI_OBSERVER_SIGNAL_EVERY_N:-20}"
