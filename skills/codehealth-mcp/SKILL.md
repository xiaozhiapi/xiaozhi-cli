...
**Opt-in (XIAOZHI):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. XIAOZHI plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during XIAOZHI install/sync with `XIAOZHI_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what XIAOZHI uses for skill auto-activation.
...
| XIAOZHI skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** XIAOZHI recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
