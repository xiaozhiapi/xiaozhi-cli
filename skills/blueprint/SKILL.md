...
This skill ships with Everything Claude Code. No separate installation is needed when XIAOZHI is installed.

### Full XIAOZHI install

If you are working from the XIAOZHI repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the XIAOZHI diff before updating:
...
If you are vendoring only this skill outside the full XIAOZHI install, copy the reviewed file from the XIAOZHI repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed XIAOZHI commit rather than running `git pull`.
