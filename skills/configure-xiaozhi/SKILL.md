...
# Configure Everything Claude Code (XIAOZHI)
...
- User says "configure xiaozhi", "install xiaozhi", "setup everything claude code", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing XIAOZHI installation
- User wants to optimize installed skills or rules for their project

## Prerequisites

This skill must be accessible to Claude Code before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install xiaozhi@xiaozhi` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-xiaozhi/SKILL.md`, then activate by saying "configure xiaozhi"

---

## Step 0: Clone XIAOZHI Repository

Before any installation, clone the latest XIAOZHI source to `/tmp`:

```bash
rm -rf /tmp/everything-claude-code
git clone https://github.com/xiaozhiapi/everything-claude-code.git /tmp/everything-claude-code
```

Set `XIAOZHI_ROOT=/tmp/everything-claude-code` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing XIAOZHI clone.
...
```
Question: "Where should XIAOZHI components be installed?"
Options:
...
`claude-api` is an Anthropic canonical skill. Install it from [`anthropics/skills`](https://github.com/anthropics/skills) when you want the official Claude API workflow instead of an XIAOZHI-bundled copy.
...
cp -R "$XIAOZHI_ROOT/.agents/skills/<skill-name>" "$TARGET/skills/"

# Niche skills live under skills/
cp -R "$XIAOZHI_ROOT/skills/<skill-name>" "$TARGET/skills/"
```
...
```bash
# Common rules
cp -r $XIAOZHI_ROOT/rules/common $TARGET/rules/common

# Language-specific rules (preserve per-language directories)
cp -r $XIAOZHI_ROOT/rules/typescript $TARGET/rules/typescript   # if selected
cp -r $XIAOZHI_ROOT/rules/python $TARGET/rules/python            # if selected
cp -r $XIAOZHI_ROOT/rules/golang $TARGET/rules/golang            # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source XIAOZHI repository (`$XIAOZHI_ROOT/`).
