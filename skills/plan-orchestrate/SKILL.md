...
description: Read a plan document, decompose it into steps, design a per-step agent chain from the XIAOZHI catalogue, and emit ready-to-paste /orchestrate custom prompts. Generative only — never invokes /orchestrate itself. Use when the user has a multi-step plan and wants to drive it through orchestrate without composing chains by hand.
metadata:
  origin: XIAOZHI
---
...
## XIAOZHI install form and namespacing
...
### Phase 0 — Detect XIAOZHI mode + language

1. Read `<plan-doc-path>`. If missing or empty, report and stop.
2. Detect XIAOZHI install form once and freeze it into `XIAOZHI_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<claude-home>/plugins/marketplaces/everything-claude-code/` exists → `XIAOZHI_MODE=plugin`.
   2. Else if `<claude-home>/agents/` exists and contains at least one XIAOZHI agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `XIAOZHI_MODE=legacy`.
   3. Else → default to `XIAOZHI_MODE=legacy` and emit a one-line warning at the top of the output: `> Warning: could not detect XIAOZHI install; defaulting to legacy form. If you use the plugin install, edit the prefixes manually.`
...
Emit Markdown using **the form determined by `XIAOZHI_MODE`**. The output uses one form throughout — every `{ORCH_CMD}` and every agent name is rendered with the matching prefix from Phase 0. **Do not emit both forms; do not include "this is plugin form" / "strip the prefix" instructions in the rendered output.**
...
- **Plan declares agents** (rare): first **strip any `everything-claude-code:` prefix** to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `XIAOZHI_MODE`.
...
If `XIAOZHI_MODE=legacy` were detected, the same step would be emitted as a single uniform command (no plugin-prefixed forms anywhere in the output):
